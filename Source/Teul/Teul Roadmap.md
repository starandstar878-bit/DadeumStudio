...
- [x] **6. 자산/의존성 preflight**: impulse, wavetable, 외부 파일 참조가 export 패키지에 포함 가능한지 검사하고 manifest 입력으로 정리
- [x] **7. EditableGraph JSON export/import 정리**: 버전, 노드 메타데이터, 연결, 파라미터, 자산 참조를 포함한 `.teul` 패키지 round-trip 경로 정리
- [x] **8. RuntimeModule 클래스 스켈레톤 생성기**: 그래프를 블랙박스 DSP 클래스로 생성하고 `prepare`, `reset`, `process`, `setParam`, `getParam` API 뼈대 제공
- [x] **9. 파라미터 표면/APVTS 생성**: `exposeToIeum`, `isAutomatable`, `enumOptions`, `range`, `step` 메타데이터를 이용해 APVTS layout 및 파라미터 접근 코드 생성
- [x] **10. `paramId` 정책 확정 및 내부 인덱스 매핑**: 외부 공개 식별자는 기존 `paramId = teul.node.<nodeId>.<paramKey>`를 유지하고, 내부는 `paramId -> index/handle`로 최적화
- [x] **11. 그래프 스케줄/버퍼 플래너 생성**: 위상 정렬, 임시 버퍼 계획, 처리 순서를 export IR에서 고정해 RuntimeModule codegen의 기반으로 사용
- [x] **12. 최적화된 DSP codegen 패스**: dead path 제거, 상수 파라미터 상수화, 불필요한 dispatch 제거, 임시 버퍼 재사용 등 export 전용 최적화 도입
- [x] **13. dry-run export report**: 노출 파라미터 수, 예상 버퍼 수, 생성 파일 목록, 제한 사항을 요약 보고서로 출력
- [x] **14. 프로젝트 편입 자동화**: 생성된 `.h/.cpp`를 Jucer/CMake 또는 현재 앱 빌드 경로에 안정적으로 포함시키는 통합 경로 제공
