    ...
    public:
      ...
      void processSamples(const TProcessContext &ctx) override {
        ...
        if (mustUpdateCoefficients) {
          updateCoefficients(cutoffFreq); // Base coefficients
        ...
        for (int i = 0; i < numSamples; ++i) {
          ...
          if (lOut != nullptr)
            lOut[i] = filters[0].processSample(lIn != nullptr ? lIn[i] : 0.0f);
          if (rOut != nullptr)
            rOut[i] = filters[1].processSample(rIn != nullptr ? rIn[i] : 0.0f);
        }
      }
