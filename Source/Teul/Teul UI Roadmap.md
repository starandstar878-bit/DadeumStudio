...
- [ ] **퍼지 검색 + 최근 사용 우선**: 완전 일치가 아니어도 결과를 찾고, 최근 삽입한 노드를 상단에 우선 노출
- [ ] **키보드 완결 플로우**: `Tab`, `↑/↓`, `Enter`, `Esc`만으로 탐색/삽입/취소 가능

#### 단계 2: 노드 라이브러리 & 삽입 배치
- [ ] **노드 라이브러리 사이드 패널**: 좌측에 카테고리별(소스, 필터, 믹서, 유틸) 노드 목록 패널. 드래그 앤 드롭으로 캔버스에 배치
- [ ] **와이어 위 삽입(Insert on Wire)**: 노드를 연결선 위에 드롭하면 기존 연결을 분할하여 자동으로 앞/뒤 재연결
- [ ] **캔버스 우클릭 추가 메뉴**: '여기에 노드 추가' 리스트 + 현재 위치 기준 즉시 생성

#### 단계 3: 멀티 셀렉션 및 정렬 도구
- [ ] **마키 선택 (Marquee)**: 드래그로 직사각형을 그려 범위 내 노드 다중 선택 후 정렬·그룹화 가능
- [ ] **정렬/간격 분배 (Align/Distribute)**: 좌/우/상/하 정렬과 일정 간격 분배를 원클릭 제공
- [ ] **스마트 스냅 가이드**: 이동 중 중심선/간격 스냅과 가이드 라인 표시

#### 단계 4: 미니맵 및 대형 그래프 내비게이션
- [ ] **Mini-Map Navigator**: Gyeol 네비게이터 패널과 동일한 개념의 그래프 전체 미니맵을 우하단에 표시
- [ ] **미니맵 클릭 점프 + 뷰포트 드래그**: 미니맵에서 클릭하면 해당 위치로 이동하고, 뷰 박스를 직접 드래그해 탐색
- [ ] **노드 검색 점프**: 노드 이름 검색 후 해당 노드로 카메라를 즉시 포커싱

#### 단계 5: 맥락 메뉴(Context Menu) 확장
- [ ] **노드 우클릭 메뉴**: 복제(Duplicate), 삭제, 바이패스 토글, 연결 모두 끊기(Disconnect All), 속성 패널 열기
- [ ] **노드 교체(Replace Node)**: 타입 호환 가능한 다른 노드로 교체하고 가능한 연결/파라미터를 유지
- [ ] **멀티 선택 일괄 명령**: 다중 선택 상태에서 일괄 바이패스, 일괄 복제, 일괄 삭제 지원

#### 단계 6: 그래프 구조화(Organization)
- [ ] **코멘트/프레임 영역**: 여러 노드를 시각적 프레임으로 묶어 패치 의도를 문서화
- [ ] **프레임 접기/잠금**: 큰 패치를 접어서 단순화하고 실수 이동을 방지
- [ ] **색상 태그/북마크**: 중요 서브그래프를 색상/북마크로 빠르게 탐색

#### 단계 7: 파워 유저 워크플로우
- [ ] **커맨드 팔레트(Command Palette)**: 기능 검색 후 즉시 실행
- [ ] **단축키 중심 편집**: 노드 추가/정렬/연결 해제 등 주요 동작의 단축키 제공
- [ ] **안전 장치 UX**: 위험 동작(대량 삭제/해제) 전 확인 및 즉시 Undo 안내
