...
#### 🛤️ Rail UX 상세 구현 계획 (Implementation Strategy)

1.  **데이터 스펙 정의 (`Document/TDocumentTypes.h`)**:
    - `TNode` 구조체에 `isRailNode` 플래그 및 `RailType` (Left/Right/Bottom) 필드 추가.
    - 레일 배치를 위한 고정 인덱스 또는 순서 메타데이터 정의.
2.  **레일 패널 구축 (`Editor/Panels/TRailPanel.*`)**:
    - 레일의 3면 레이아웃(좌/우/하단)을 관리하는 고정 컨테이너 패널 구축.
    - `TRuntimeDeviceManager::Listener`를 상속받아 물리 장치 연결 상태를 직접 구독 및 화면 갱신.
3.  **IOControl 통합 모듈 (`Editor/Renderers/TIOControlNodeRenderer.*`)**:
    - **통합 패턴**: `TRailNodeComponent` 클래스 정의와 레일 전용 렌더링 로직을 한 파일에 통합.
    - 캔버스 노드와 차별화된 슬림하고 직관적인 장치 아이콘 중심의 시나각화 구현.
4.  **Editor 레이아웃 통합 (`Editor/TTeulEditor.*`)**:
    - `TTeulEditor` 내부에 `TRailPanel`을 배치하고 캔버스 영역(Viewport)과 겹치지 않게 레이아웃 고정.
    - 레일 노드에서 캔버스로의 드래그 바인딩 인터랙션 연결.
