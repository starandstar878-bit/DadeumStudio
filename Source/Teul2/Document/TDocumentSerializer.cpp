...
const TSystemRailEndpoint *findRailEndpointByKind(const TTeulDocument &doc,
...
bool tryMapLegacySourcePort(const TTeulDocument &doc,
                            const TNode &node,
                            PortId portId,
                            TEndpoint &mappedOut) {
...
bool tryMapLegacySinkPort(const TTeulDocument &doc,
                          const TNode &node,
                          PortId portId,
                          TEndpoint &mappedOut) {
  const auto *port = node.findPort(portId);
...
void normalizeLegacyRailBridgeNodes(TTeulDocument &doc,
...
bool TDocumentSerializer::usesLegacyDocumentAliases(const juce::var &json) {
...
juce::var TDocumentSerializer::migrateDocumentJson(
    const juce::var &json,
...
juce::var TDocumentSerializer::normalizeDocumentJson(const juce::var &json) {
  if (!json.isObject())
    return {};
  ...
  object->setProperty(
      ...
      migrateArray(propertyOrAlias(source, {"nodes", "node_list"}),
                   [](const juce::var &item) {
                     return TDocumentSerializer::migrateNodeJson(item);
                   }));
  object->setProperty(
      "connections",
      migrateArray(propertyOrAlias(source, {"connections", "connection_list"}),
                   [](const juce::var &item) {
                     return TDocumentSerializer::migrateConnectionJson(item);
                   }));
  object->setProperty(
      "frames",
      migrateArray(propertyOrAlias(source, {"frames", "frame_regions"}),
                   [](const juce::var &item) {
                     return TDocumentSerializer::migrateFrameJson(item);
                   }));
  object->setProperty(
      "bookmarks",
      migrateArray(propertyOrAlias(source, {"bookmarks", "bookmark_list"}),
                   [](const juce::var &item) {
                     return TDocumentSerializer::migrateBookmarkJson(item);
                   }));
  object->setProperty(
      "control_state",
...
juce::var TDocumentSerializer::migrateDocumentV1ToV2(const juce::var &json) {
  if (!json.isObject())
    return {};
  ...
  object->setProperty(
      ...
      migrateArray(propertyOrAlias(source, {"frames", "frame_regions"}),
                   [](const juce::var &item) {
                     return TDocumentSerializer::migrateFrameJsonV2(item);
                   }));
  object->setProperty(
      "bookmarks",
      migrateArray(propertyOrAlias(source, {"bookmarks", "bookmark_list"}),
...
juce::var TDocumentSerializer::migrateDocumentV2ToV3(const juce::var &json) {
  if (!json.isObject())
    return {};
...
juce::var TDocumentSerializer::migrateMetaJson(const juce::var &json) {
  if (!json.isObject())
...
juce::var TDocumentSerializer::migrateNodeJson(const juce::var &json) {
  if (!json.isObject())
    return {};
  ...
  object->setProperty(
      ...
      migrateArray(propertyOrAlias(source, {"ports", "port_list"}),
                   [](const juce::var &item) { return TDocumentSerializer::migratePortJson(item); }));
  return juce::var(object);
}

juce::var TDocumentSerializer::migratePortJson(const juce::var &json) {
  if (!json.isObject())
    return {};
...
juce::var TDocumentSerializer::migrateConnectionJson(const juce::var &json) {
  if (!json.isObject())
    return {};
...
juce::var TDocumentSerializer::migrateFrameJsonV2(const juce::var &json) {
  if (!json.isObject())
    return {};
...
juce::var TDocumentSerializer::migrateFrameJson(const juce::var &json) {
  if (!json.isObject())
    return {};
...
juce::var TDocumentSerializer::migrateBookmarkJson(const juce::var &json) {
  if (!json.isObject())
    return {};
