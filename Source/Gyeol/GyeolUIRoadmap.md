...
- [x] **19-a. 4단계 브레이크포인트 클래스 정의**: >=1600, 1200~1599, 900~1199, <900.
- [x] **19-b. 자동 도크 리플로우 구현**: 최소 너비/높이 제약을 유지하면서 창 크기 변경 시 레이아웃 재배치.
- [x] **19-c. 내로우 오버레이 모드 추가**: 좁은 너비에서 사이드 도크를 임시 오버레이로 전환.
- [x] **19-d. 프로필별 레이아웃 저장**: 브레이크포인트 클래스별로 도크 상태 저장/복원.

### 20. 적응형 밀도 시스템
- [x] **20-a. 밀도 값 토큰화**: 간격, 행 높이, 컨트롤 높이, 타이포그래피 스케일 토큰화.
- [x] **20-b. 밀도 프리셋 3종 제공**: Compact / Comfortable / Spacious.
- [x] **20-c. 너비 기반 컴팩션 규칙 추가**: 2열→1열 폴백, 아이콘화 라벨, 팝오버 에디터 전환.
- [x] **20-d. HiDPI 동작 검증**: 125%/150%/200% 스케일에서 잘림 현상 없는지 확인.

### 21. 커맨드 팔레트 및 키보드 우선 워크플로우
- [x] **21-a. 커맨드 팔레트 구현 (Ctrl+K)**: 액션 검색 및 실행.
- [x] **21-b. 퀵 스위처 구현 (Ctrl+P)**: 위젯/패널/섹션으로 빠른 이동.
- [x] **21-c. 키보드 내비게이션 계약 표준화**: Tab, Enter, Escape, Arrow 동작 일관화.
- [x] **21-d. 단축키 충돌 감지 및 에디터 추가**.

### 22. 테마 변형 및 접근성
- [x] **22-a. 3가지 테마 추가**: Deep Dark, Light, High Contrast.
- [ ] **22-b. 모션 감소 옵션 추가**: 전역 애니메이션 강도 및 지속 시간 스케일 조절.
- [x] **22-c. 접근성 검사 추가**: 명도 대비, 포커스 가시성, 히트 타겟 크기 검증.
...
- [x] **23-d. 저사양 렌더링 품질 모드 추가**.

### 24. 시각적 회귀 및 UI 품질 게이트
- [x] **24-a. 주요 패널 기준 스냅샷 캡처**.
- [ ] **24-b. 허용 오차 임계값을 가진 픽셀 Diff CI 검사 추가**.
- [x] **24-c. 다중 뷰포트 프로필에서 스냅샷 테스트 실행**.
