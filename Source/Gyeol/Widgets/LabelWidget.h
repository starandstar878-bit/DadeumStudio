...
public:
  WidgetDescriptor makeDescriptor() const override {
    ...
    descriptor.painter = [](juce::Graphics &g, const WidgetModel &widget,
      ...
      const auto text =
          widget.properties.getWithDefault("text", juce::var("Label"))
              .toString();
    ...
    descriptor.iconPainter = [](juce::Graphics &g,
      ...
      g.setColour(juce::Colour::fromRGB(220, 180, 140));
      g.setFont(juce::FontOptions(16.0f, juce::Font::bold));
      g.drawFittedText(juce::String("Aa"), r.toNearestInt(),
    ...
    descriptor.dropOptions = [](const WidgetModel &, const AssetRef &asset) {
      if (asset.mime.startsWithIgnoreCase("text/")) {
        return std::vector<DropOption>{
            {"Text", juce::Identifier("text"),
             "Use dropped text asset id as label text source"}};
      }

      if (asset.mime.startsWithIgnoreCase("image/")) {
        return std::vector<DropOption>{
            {"Background Image", juce::Identifier("label.backgroundImage"),
             "Apply image to label background"}};
      }
