...
public:
  WidgetDescriptor makeDescriptor() const override {
    ...
    descriptor.painter = [](juce::Graphics &g, const WidgetModel &widget,
      ...
      items.addLines(widget.properties
                         .getWithDefault("combo.items",
                                         juce::var("Item 1\nItem 2\nItem 3"))
                         .toString());
      ...
      auto selectedIndex = static_cast<int>(widget.properties.getWithDefault(
          "combo.selectedIndex", juce::var(1)));
    ...
    descriptor.exportCodegen = [](const ExportCodegenContext &context,
      ...
      juce::StringArray items;
      items.addLines(context.widget.properties
                         .getWithDefault("combo.items",
                                         juce::var("Item 1\nItem 2\nItem 3"))
                         .toString());
      for (int i = items.size() - 1; i >= 0; --i) {
      ...
      auto selectedIndex =
          static_cast<int>(context.widget.properties.getWithDefault(
              "combo.selectedIndex", juce::var(1)));
      selectedIndex = juce::jlimit(0, std::max(0, items.size()), selectedIndex);
      const auto editable =
          static_cast<bool>(context.widget.properties.getWithDefault(
              "combo.editable", juce::var(false)));
      ...
      for (int i = 0; i < items.size(); ++i) {
        out.constructorLines.add("    " + context.memberName + ".addItem(" +
                                 toLiteral(items[i]) + ", " +
                                 juce::String(i + 1) + ");");
      }
      out.constructorLines.add(
          "    " + context.memberName + ".setEditableText(" +
          juce::String(editable ? "true" : "false") + ");");
      out.constructorLines.add("    " + context.memberName +
                               ".setTextWhenNothingSelected(\"Select...\");");
      if (selectedIndex > 0) {
        out.constructorLines.add(
            "    " + context.memberName + ".setSelectedId(" +
            juce::String(selectedIndex) + ", juce::dontSendNotification);");
      }
