...
public:
  WidgetDescriptor makeDescriptor() const override {
    ...
    descriptor.painter = [](juce::Graphics &g, const WidgetModel &widget,
      ...
      auto value =
          static_cast<float>(readNumeric("value", (rangeMin + rangeMax) * 0.5));
      ...
      const auto angle =
          juce::jmap<float>(normalized, 0.0f, 1.0f, minAngle, maxAngle);

      const auto c = knob.getCentre();
      const auto r = knob.getWidth() * 0.34f;
      g.drawLine(c.x, c.y, c.x + std::cos(angle) * r, c.y + std::sin(angle) * r,
                 2.0f);
    };
    ...
    descriptor.exportCodegen = [](const ExportCodegenContext &context,
      ...
      const auto styleToLiteral =
        ...
        static const std::array<std::pair<const char *, const char *>, 4>
            mapping{
                std::pair<const char *, const char *>{"rotary",
                                                      "juce::Slider::Rotary"},
      ...
      auto styleKey =
          context.widget.properties
              .getWithDefault("knob.style", juce::var("rotaryVerticalDrag"))
              .toString()
              .trim();
      if (styleKey.isEmpty())
      ...
      out.constructorLines.add("    " + context.memberName +
                               ".setSliderStyle(" + styleToLiteral(styleKey) +
                               ");");
    ...
    descriptor.hitTest = [](const WidgetModel &widget,
                            juce::Point<float> local) {
      const auto center = juce::Point<float>(widget.bounds.getWidth() * 0.5f,
                                             widget.bounds.getHeight() * 0.5f);
      const auto radius =
          std::min(widget.bounds.getWidth(), widget.bounds.getHeight()) * 0.5f;
    ...
    descriptor.dropOptions = [](const WidgetModel &, const AssetRef &asset) {
      ...
      return std::vector<DropOption>{{"Cap Image",
                                      juce::Identifier("knob.capImage"),
                                      "Apply image to knob cap"}};
    };
