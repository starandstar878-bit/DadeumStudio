...
public:
  WidgetDescriptor makeDescriptor() const override {
    ...
    descriptor.painter = [](juce::Graphics &g, const WidgetModel &widget,
      ...
      const auto readNumeric = [&widget](const juce::Identifier &key,
                                         double fallback) -> double {
        const auto raw =
            widget.properties.getWithDefault(key, juce::var(fallback));
      ...
      const auto isRotaryStyle = [](const juce::String &styleKey) noexcept {
        return styleKey == "rotary" || styleKey == "rotaryHorizontalDrag" ||
               styleKey == "rotaryVerticalDrag" ||
               styleKey == "rotaryHorizontalVerticalDrag";
      };
      const auto normalizeSliderStyle =
          [isRotaryStyle](juce::String styleKey) -> juce::String {
      ...
      const auto clampToRange = [rangeMin, rangeMax](float v) noexcept {
        return juce::jlimit(rangeMin, rangeMax,
                            std::isfinite(v) ? v : rangeMin);
      };

      auto value = clampToRange(static_cast<float>(
          readNumeric("value", (rangeMin + rangeMax) * 0.5)));
      auto minValue =
          clampToRange(static_cast<float>(readNumeric("minValue", rangeMin)));
      auto maxValue =
          clampToRange(static_cast<float>(readNumeric("maxValue", rangeMax)));
      ...
      const auto drawThumb = [&g](float x, float y, juce::Colour color,
        ...
        g.setColour(color);
        g.fillEllipse(x - diameter * 0.5f, y - diameter * 0.5f, diameter,
                      diameter);
      };
      ...
      if (isVerticalStyle(normalizedStyle)) {
        ...
        if (isTwoValueStyle(normalizedStyle) ||
            isThreeValueStyle(normalizedStyle)) {
    ...
    descriptor.exportCodegen = [](const ExportCodegenContext &context,
                                  ExportCodegenOutput &out) -> juce::Result {
      const auto readNumeric = [&context](const juce::Identifier &key,
                                          double fallback) -> double {
        const auto raw =
            context.widget.properties.getWithDefault(key, juce::var(fallback));
        if (!raw.isInt() && !raw.isInt64() && !raw.isDouble())
          return fallback;
      ...
      const auto clampRange = [](double minValue, double maxValue,
                                 double value) noexcept {
        if (!std::isfinite(value))
          return minValue;
        return juce::jlimit(minValue, maxValue, value);
      };
      const auto styleToLiteral =
          [](const juce::String &styleKey) -> juce::String {
        static const std::array<std::pair<const char *, const char *>, 9>
            mapping{
                std::pair<const char *, const char *>{
                    "linearHorizontal", "juce::Slider::LinearHorizontal"},
      ...
      auto value =
          clampRange(rangeMin, rangeMax,
                     readNumeric("value", (rangeMin + rangeMax) * 0.5));
      auto minValue =
          clampRange(rangeMin, rangeMax, readNumeric("minValue", rangeMin));
      auto maxValue =
          clampRange(rangeMin, rangeMax, readNumeric("maxValue", rangeMax));
      if (minValue > maxValue)
        std::swap(minValue, maxValue);
      value = juce::jlimit(minValue, maxValue, value);
      ...
      out.constructorLines.add("    " + context.memberName +
                               ".setSliderStyle(" + styleLiteral + ");");
      out.constructorLines.add(
          "    " + context.memberName +
          ".setTextBoxStyle(juce::Slider::NoTextBox, false, 0, 0);");
      out.constructorLines.add("    " + context.memberName + ".setRange(" +
                               juce::String(rangeMin, 8) + ", " +
                               juce::String(rangeMax, 8) + ", " +
                               juce::String(step, 8) + ");");

      if (isThreeValueStyle(styleKey)) {
        out.constructorLines.add(
            "    " + context.memberName + ".setMinAndMaxValues(" +
            juce::String(minValue, 8) + ", " + juce::String(maxValue, 8) +
            ", juce::dontSendNotification);");
        out.constructorLines.add("    " + context.memberName + ".setValue(" +
                                 juce::String(value, 8) +
                                 ", juce::dontSendNotification);");
    ...
    descriptor.dropOptions = [](const WidgetModel &, const AssetRef &asset) {
      ...
      return std::vector<DropOption>{
          {"Thumb Image", juce::Identifier("slider.thumbImage"),
           "Apply image to thumb"},
          {"Track Image", juce::Identifier("slider.trackImage"),
           "Apply image to track"}};
    };
