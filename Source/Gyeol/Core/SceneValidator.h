...
enum class RuntimeBindingIssueSeverity { warning, error };
...
inline bool isFiniteBounds(const juce::Rectangle<float> &bounds) noexcept {
  return std::isfinite(bounds.getX()) && std::isfinite(bounds.getY()) &&
         std::isfinite(bounds.getWidth()) && std::isfinite(bounds.getHeight());
}

inline bool
isFiniteRectNonNegative(const juce::Rectangle<float> &bounds) noexcept {
  return isFiniteBounds(bounds) && bounds.getWidth() >= 0.0f &&
...
inline juce::Result validateDocument(const DocumentModel &document) {
  ...
  for (const auto &widget : document.widgets) {
    ...
    if (propertyResult.failed())
      return juce::Result::fail("invalid widget.properties for widget id " +
    ...
    for (const auto memberId : group.memberWidgetIds) {
      ...
      if (widgetIdSet.count(memberId) == 0)
        return juce::Result::fail(
            "group member id not found in document widgets");
      if (!membersInGroup.insert(memberId).second)
        return juce::Result::fail(
            "group member ids must be unique inside each group");
  ...
  for (const auto &group : document.groups) {
    ...
    if (groupById.find(*group.parentGroupId) == groupById.end())
      return juce::Result::fail(
          "group.parentGroupId not found in document groups");

    ++childCounts[*group.parentGroupId];
  }

  std::unordered_map<WidgetId, std::unordered_set<WidgetId>>
      expectedChildGroupIdsByParent;
  ...
  for (const auto &group : document.groups) {
    ...
    for (const auto childGroupId : group.memberGroupIds) {
      ...
      if (groupIds.count(childGroupId) == 0)
        return juce::Result::fail(
            "group.memberGroupIds id not found in document groups");
      if (!explicitChildGroupIds.insert(childGroupId).second)
        return juce::Result::fail(
            "group.memberGroupIds must be unique inside each group");
    }
    ...
    if (explicitChildGroupIds != expectedChildIds)
      return juce::Result::fail(
          "group.memberGroupIds must match parentGroupId hierarchy");
  }
  ...
  for (const auto &group : document.groups) {
    ...
    const auto childCount =
        childCountIt != childCounts.end() ? childCountIt->second : 0;

    // Keep leaf-group invariants stable: a leaf group should have at least two
    // widgets.
    if (childCount == 0 && group.memberWidgetIds.size() < 2)
      return juce::Result::fail(
          "leaf group must contain at least two widget ids");
  }
  ...
  for (const auto &layer : document.layers) {
    ...
    if (widgetIdSet.count(layer.id) > 0 || groupIds.count(layer.id) > 0)
      return juce::Result::fail(
          "layer.id must not collide with widget/group ids");
    ...
    for (const auto memberId : layer.memberWidgetIds) {
      ...
      if (widgetIdSet.count(memberId) == 0)
        return juce::Result::fail(
            "layer.members id not found in document widgets");
      if (!membersInLayer.insert(memberId).second)
        return juce::Result::fail(
            "layer.members ids must be unique inside each layer");
    ...
    for (const auto groupId : layer.memberGroupIds) {
      ...
      if (groupIds.count(groupId) == 0)
        return juce::Result::fail(
            "layer.memberGroups id not found in document groups");
      if (!groupsInLayer.insert(groupId).second)
        return juce::Result::fail(
            "layer.memberGroups ids must be unique inside each layer");
    ...
    if (ownerIt != ownerByWidgetId.end()) {
      ...
      while (cursor > kRootId && visited.insert(cursor).second) {
        ...
        if (groupIt == groupById.end() ||
            !groupIt->second->parentGroupId.has_value())
          break;
  ...
  for (const auto &asset : document.assets) {
    ...
    if (metaResult.failed())
      return juce::Result::fail("invalid asset.meta for asset id " +
                                juce::String(asset.id) + ": " +
...
inline juce::Result validateEditorState(const DocumentModel &document,
  ...
  if (std::adjacent_find(sortedSelection.begin(), sortedSelection.end()) !=
      sortedSelection.end())
  ...
  for (const auto &param : document.runtimeParams) {
    ...
    case RuntimeParamValueType::number: {
      if (!isNumericVar(param.defaultValue))
        return juce::Result::fail(
            "runtimeParams.defaultValue must be numeric for number type");
      const auto value = static_cast<double>(param.defaultValue);
      if (!std::isfinite(value))
        return juce::Result::fail(
            "runtimeParams.defaultValue must be finite for number type");
      break;
    }
    case RuntimeParamValueType::boolean:
      if (!param.defaultValue.isBool())
        return juce::Result::fail(
            "runtimeParams.defaultValue must be bool for boolean type");
      break;
    case RuntimeParamValueType::string:
      if (!param.defaultValue.isString())
        return juce::Result::fail(
            "runtimeParams.defaultValue must be string for string type");
      break;
    }
  }
...
inline juce::Result validatePropertyBindings(const DocumentModel &document) {
  ...
  for (const auto &param : document.runtimeParams) {
    const auto key = param.key.trim();
    if (key.isNotEmpty() &&
        runtimeParamValues.find(key) == runtimeParamValues.end())
  ...
  for (const auto &binding : document.propertyBindings) {
    ...
    if (binding.targetWidgetId <= kRootId)
      return juce::Result::fail(
          "propertyBindings.targetWidgetId must be > rootId");
    ...
    if (targetProperty.isEmpty())
      return juce::Result::fail(
          "propertyBindings.targetProperty must not be empty");
    if (!isIdentifierLike(targetProperty)) {
      return juce::Result::fail("propertyBindings.targetProperty has invalid "
                                "identifier for binding id " +
    ...
    if (expression.isEmpty())
      return juce::Result::fail(
          "propertyBindings.expression must not be empty");

    const auto targetPropertyId = juce::Identifier(targetProperty);
    if (!targetWidget.properties.contains(targetPropertyId)) {
      return juce::Result::fail("propertyBindings.targetProperty not found on "
                                "target widget for binding id " +
                                juce::String(binding.id));
    }
    ...
    if (!(currentValue.isBool() || isNumericVar(currentValue) ||
      ...
      return juce::Result::fail(
          "propertyBindings.targetProperty type mismatch for binding id " +
          juce::String(binding.id) + " (number/integer/boolean/string only)");
    }

    if (runtimeParamValues.find(expression) == runtimeParamValues.end()) {
      const auto evaluation =
          Runtime::PropertyBindingResolver::evaluateExpression(
              expression, runtimeParamValues);
      if (!evaluation.success) {
        return juce::Result::fail(
            "propertyBindings.expression invalid for binding id " +
            juce::String(binding.id) + ": " + evaluation.error);
      }
    }
  }
...
inline std::vector<RuntimeBindingIssue>
validateRuntimeBindings(const DocumentModel &document) {
  ...
  auto hasNode = [&widgetIds, &groupIds,
                  &layerIds](const NodeRef &node) noexcept {
  ...
  for (const auto &binding : document.runtimeBindings) {
    ...
    if (binding.id <= kRootId) {
      pushIssue(RuntimeBindingIssueSeverity::error, "Invalid binding id",
                bindingLabel + " has invalid id");
    } else if (!bindingIds.insert(binding.id).second) {
      pushIssue(RuntimeBindingIssueSeverity::error, "Duplicate binding id",
                bindingLabel + " is duplicated");
    }

    if (binding.sourceWidgetId <= kRootId) {
      pushIssue(RuntimeBindingIssueSeverity::error, "Invalid source widget id",
                bindingLabel + " has invalid sourceWidgetId");
    } else if (widgetIds.count(binding.sourceWidgetId) == 0) {
      pushIssue(RuntimeBindingIssueSeverity::warning, "Missing source widget",
                bindingLabel + " references sourceWidgetId=" +
                    juce::String(binding.sourceWidgetId) +
                    " which does not exist");
    }

    if (binding.eventKey.trim().isEmpty()) {
      pushIssue(RuntimeBindingIssueSeverity::error, "Missing event key",
                bindingLabel + " has empty eventKey");
    }

    if (binding.actions.empty()) {
      pushIssue(RuntimeBindingIssueSeverity::warning, "Empty action chain",
    ...
    for (size_t actionIndex = 0; actionIndex < binding.actions.size();
      ...
      const auto actionLabel = bindingLabel + " action#" +
      ...
      case RuntimeActionKind::setNodeProps: {
        if (action.target.id <= kRootId) {
          ...
          pushIssue(RuntimeBindingIssueSeverity::warning, "Missing target node",
        ...
        if (action.opacity.has_value()) {
          if (action.target.kind == NodeKind::layer) {
            pushIssue(
                RuntimeBindingIssueSeverity::error, "Invalid action payload",
        ...
        if (patchResult.failed()) {
          pushIssue(RuntimeBindingIssueSeverity::error,
                    "Invalid action payload",
                    actionLabel +
                        " patch is invalid: " + patchResult.getErrorMessage());
        }
        break;
      }
      ...
      case RuntimeActionKind::setNodeBounds: {
        if (action.targetWidgetId <= kRootId) {
          ...
          if (groupIds.count(action.targetWidgetId) > 0 ||
              layerIds.count(action.targetWidgetId) > 0) {
            pushIssue(RuntimeBindingIssueSeverity::error,
                      "Invalid action payload",
        ...
        if (!isFiniteRectNonNegative(action.bounds)) {
          pushIssue(
              RuntimeBindingIssueSeverity::error, "Invalid action payload",
              actionLabel +
                  " setNodeBounds.bounds must be finite and non-negative");
        }
        break;
      }
      }
...
inline juce::Result
validateScene(const DocumentModel &document,
