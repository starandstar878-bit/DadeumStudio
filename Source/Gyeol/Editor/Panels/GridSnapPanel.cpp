...
namespace
{
    ...
    {
        if (auto* lf = dynamic_cast<const Gyeol::GyeolCustomLookAndFeel*>(&component.getLookAndFeel()); lf != nullptr)
    ...
    void configureDragNumberSlider(juce::Slider& slider,
                                   double min,
                                   double max,
                                   double step,
                                   double defaultValue)
    {
        ...
        slider.setDoubleClickReturnValue(true, defaultValue, juce::ModifierKeys::noModifiers);
    }
}
...
namespace Gyeol::Ui::Panels
{
    GridSnapPanel::GridSnapPanel()
    {
        ...
        for (auto* toggle : { &snapEnabledToggle, &smartSnapToggle, &gridToggle, &gridSnapToggle })
        ...
        gridSizeBadgeLabel.setColour(juce::Label::textColourId, palette(GyeolPalette::AccentPrimary));
    ...
    void GridSnapPanel::paint(juce::Graphics& g)
    {
        ...
        g.drawText("Grid / Snap", titleBounds, juce::Justification::centredLeft, true);

        g.setColour(palette(GyeolPalette::TextSecondary, 0.9f));
        g.setFont(makePanelFont(*this, 10.0f, true));
        g.drawText("MODES", gridSectionLabelBounds, juce::Justification::centredLeft, true);
        g.drawText("SNAP", snapSectionLabelBounds, juce::Justification::centredLeft, true);
    ...
    void GridSnapPanel::resized()
    {
        ...
        titleBounds = area.removeFromTop(18);
        ...
        const auto leftToggleWidth = juce::jmax(32, (topRow.getWidth() - kToggleGap) / 2);
    ...
    void GridSnapPanel::syncUiFromSettings()
    {
        ...
        gridSizeSlider.setValue(juce::jlimit(1.0, 100.0, static_cast<double>(snapSettings.gridSize)), juce::dontSendNotification);
        toleranceSlider.setValue(juce::jlimit(0.0, 20.0, static_cast<double>(snapSettings.tolerance)), juce::dontSendNotification);
        updateGridSizeBadge();
    }
    ...
    {
        const auto valueText = juce::String(juce::roundToInt(gridSizeSlider.getValue())) + " px";
