...
juce::Rectangle<float> unionRect(const juce::Rectangle<float> &lhs,
                                 const juce::Rectangle<float> &rhs) {
...
void PropertyPanel::setInspectorTarget(InspectorTarget target) {
  ...
  target.widgetIds.erase(
      std::unique(target.widgetIds.begin(), target.widgetIds.end()),
      target.widgetIds.end());
...
void PropertyPanel::resized() {
  ...
  if (showAdvancedToggle.isVisible()) {
    showAdvancedToggle.setBounds(bounds.removeFromTop(24));
...
void PropertyPanel::layoutContent() {
  ...
  for (const auto &entry : layoutEntries) {
    ...
    if (entry.fullWidth) {
      ...
      if (entry.right != nullptr)
        entry.right->setBounds(6 + labelWidth + gap, y,
                               std::max(20, width - labelWidth - gap - 12),
                               entry.height);
    }
...
void PropertyPanel::addSectionHeader(const juce::String &text) {
  const bool isExpanded =
      collapsedSections.find(text) == collapsedSections.end();
  currentSectionExpanded = isExpanded;
  ...
  button->onClick = [this, text] {
    if (collapsedSections.count(text) > 0) {
      collapsedSections.erase(text);
    } else {
      collapsedSections.insert(text);
    }
    rebuildContent();
  };
...
void PropertyPanel::addInfoRow(const juce::String &label,
...
void PropertyPanel::addExpanderRow(const juce::String &label, bool expanded,
...
void PropertyPanel::addEditorRow(
    const Widgets::WidgetPropertySpec &spec, const ValueState &valueState,
...
PropertyPanel::ValueState
PropertyPanel::makeBooleanState(const std::vector<bool> &values) const {
  ValueState state;
  if (values.empty())
    return state;
  ...
  state.mixed = std::any_of(
      values.begin(), values.end(),
      [first = values.front()](bool value) { return value != first; });
  return state;
}

PropertyPanel::ValueState
PropertyPanel::makeFloatState(const std::vector<float> &values) const {
  ValueState state;
  if (values.empty())
    return state;
  ...
  state.valid = true;
  state.value = values.front();
  state.mixed = std::any_of(values.begin(), values.end(),
                            [first = values.front()](float value) {
                              return !floatNearlyEqual(value, first);
                            });
  return state;
}

PropertyPanel::ValueState
PropertyPanel::makeStringState(const std::vector<juce::String> &values) const {
  ValueState state;
  if (values.empty())
    return state;
  ...
  state.mixed =
      std::any_of(values.begin(), values.end(),
                  [first = values.front()](const juce::String &value) {
                    return value != first;
                  });
  return state;
}

std::optional<PropertyPanel::WidgetRef>
PropertyPanel::findWidgetRef(WidgetId id) const {
...
bool PropertyPanel::varEquals(const juce::var &lhs,
                              const juce::var &rhs) const {
...
bool PropertyPanel::applySetPropsPreview(const SetPropsAction &action,
                                         const juce::String &editKey) {
  if (!beginEditSession(editKey))
    return false;

  const auto ok = commitCallbacks.onSetPropsPreview
                      ? commitCallbacks.onSetPropsPreview(action).wasOk()
...
bool PropertyPanel::applySetBoundsPreview(const SetBoundsAction &action,
                                          const juce::String &editKey) {
  if (!beginEditSession(editKey))
    return false;

  const auto ok = commitCallbacks.onSetBoundsPreview
                      ? commitCallbacks.onSetBoundsPreview(action).wasOk()
...
std::vector<PropertyPanel::WidgetRef>
PropertyPanel::resolveTargetWidgets() const {
  ...
  if (inspectorTarget.kind == InspectorTargetKind::group) {
    const auto widgetIds =
        collectGroupWidgetIdsRecursive(inspectorTarget.nodeId);
...
std::vector<WidgetId>
PropertyPanel::collectGroupWidgetIdsRecursive(WidgetId groupId) const {
...
std::optional<juce::Rectangle<float>> PropertyPanel::computeUnionBounds(
...
PropertyPanel::resolveCurrentTransformBounds() const {
  if (inspectorTarget.kind == InspectorTargetKind::widgetSingle) {
    const auto ref = findWidgetRef(inspectorTarget.nodeId);
    return ref.has_value() ? std::optional<juce::Rectangle<float>>{ref->bounds}
                           : std::nullopt;
  }
...
PropertyPanel::clampWidgetBounds(const WidgetRef &widget,
  ...
  bounds.setX(
      juce::jlimit(0.0f, canvasWidth - bounds.getWidth(), bounds.getX()));
  bounds.setY(
      juce::jlimit(0.0f, canvasHeight - bounds.getHeight(), bounds.getY()));
  return bounds;
}
bool PropertyPanel::buildScaledBoundsUpdates(
    const std::vector<WidgetRef> &widgets,
  ...
  for (const auto &widget : widgets) {
    ...
    if (canScaleX) {
      const auto normLeft =
          (widget.bounds.getX() - sourceUnion.getX()) / sourceW;
      ...
      next.setX(widget.bounds.getX() +
                (targetUnion.getX() - sourceUnion.getX()));
    }

    if (canScaleY) {
      const auto normTop =
          (widget.bounds.getY() - sourceUnion.getY()) / sourceH;
      ...
      next.setY(widget.bounds.getY() +
                (targetUnion.getY() - sourceUnion.getY()));
    }
    ...
    if (std::abs(next.getX() - widget.bounds.getX()) <= kValueEpsilon &&
        ...
        std::abs(next.getHeight() - widget.bounds.getHeight()) <=
            kValueEpsilon) {
      continue;
    }
...
bool PropertyPanel::applyTransformPreview(
    const juce::Rectangle<float> &targetBounds, const juce::String &editKey) {
  ...
  if (inspectorTarget.kind == InspectorTargetKind::group &&
      commitCallbacks.onGroupTransformPreview) {
    if (!beginEditSession(editKey))
      return false;

    const auto result = commitCallbacks.onGroupTransformPreview(
        inspectorTarget.nodeId, targetBounds);
    if (result.failed())
      return false;
...
PropertyPanel::ValueState
PropertyPanel::makeWidgetPropertyState(const std::vector<WidgetRef> &widgets,
  ...
  for (const auto &widget : widgets) {
    ...
    if (widget.properties.contains(key)) {
      value = widget.properties[key];
                   ...
                   widgetFactory.descriptorFor(widget.type);
               descriptor != nullptr &&
               descriptor->defaultProperties.contains(key)) {
...
std::vector<Widgets::WidgetPropertySpec> PropertyPanel::commonPropertySpecs(
    const std::vector<WidgetRef> &widgets) const {
  if (widgets.empty())
    return {};

  const auto *firstDescriptor =
      widgetFactory.descriptorFor(widgets.front().type);
  ...
  std::sort(specs.begin(), specs.end(),
            [](const Widgets::WidgetPropertySpec &lhs,
               const Widgets::WidgetPropertySpec &rhs) {
...
void PropertyPanel::buildNoneContent() {
  ...
  // Phase 3: Îπ??ÅÌÉú(Empty State) - ÎØ∏Î†§???àÎÇ¥ UI
  ...
  public:
    void paint(juce::Graphics &g) override {
      ...
      // Ïª§ÏÑú ?ÑÏù¥ÏΩ?(Í∞ÑÏÜå?îÎêú ?îÏÇ¥???ïÌÉú)
      juce::Path cursorPath;
      cursorPath.startNewSubPath(centerX - 8.0f, centerY - 16.0f);
      cursorPath.lineTo(centerX - 8.0f, centerY + 8.0f);
      cursorPath.lineTo(centerX - 3.0f, centerY + 4.0f);
      cursorPath.lineTo(centerX + 2.0f, centerY + 12.0f);
      cursorPath.lineTo(centerX + 5.0f, centerY + 10.0f);
      cursorPath.lineTo(centerX + 0.0f, centerY + 2.0f);
      cursorPath.lineTo(centerX + 6.0f, centerY + 0.0f);
  ...
  auto renderer = std::make_unique<EmptyStateRenderer>();
  renderer->setSize(200, 100);
  content.addAndMakeVisible(*renderer);
  LayoutEntry entry;
  entry.left = renderer.get();
  entry.right = nullptr;
  entry.height = 100;
  entry.fullWidth = true;
  layoutEntries.push_back(entry);
  ownedEditors.push_back(std::move(renderer));
...
void PropertyPanel::buildLayerContent(const LayerModel &layer) {
  ...
  const auto addLayerText =
      [this, layerId = layer.id](
        ...
        auto apply = [this, layerId, assign,
                      editKey](const juce::var &nextValue) -> bool {
  ...
  addLayerText("Name", layer.name, "layer.name",
               [](LayerPropsPatch &patch, const juce::var &value) {
                 patch.name = value.toString();
               });

  const auto addLayerBool =
      [this, layerId = layer.id](
          const juce::String &label, bool current,
          const juce::String &keyPrefix,
  ...
  addLayerBool(
      "Visible", layer.visible, "layer.visible",
      [](LayerPropsPatch &patch, bool value) { patch.visible = value; });
  addLayerBool(
      "Locked", layer.locked, "layer.locked",
      [](LayerPropsPatch &patch, bool value) { patch.locked = value; });
...
void PropertyPanel::buildGroupContent(const GroupModel &group) {
  ...
  const auto addGroupBool =
      [this, groupId = group.id](
        ...
        auto apply = [this, groupId, assign,
                      editKey](const juce::var &nextValue) -> bool {
          SetPropsAction action;
  ...
  auto applyGroupName = [this, groupId = group.id,
                         groupNameKey](const juce::var &nextValue) -> bool {
    SetPropsAction action;
    action.kind = NodeKind::group;
    action.ids = {groupId};
    GroupPropsPatch patch;
  ...
  addEditorRow(
      nameSpec, nameState,
      [applyGroupName](const juce::var &nextValue) {
        applyGroupName(nextValue);
      },
  ...
  addGroupBool(
      ...
      [](GroupPropsPatch &patch, bool value) { patch.visible = value; });
  addGroupBool(
      "Locked", group.locked, "group.locked",
      [](GroupPropsPatch &patch, bool value) { patch.locked = value; });
  addSectionHeader("Transform");
  if (const auto transform = resolveCurrentTransformBounds();
      transform.has_value()) {
    const auto addTransformField =
        [this, groupId = group.id](const juce::String &label, int axisIndex,
                                   float current) {
          Widgets::WidgetPropertySpec spec;
          ...
          const auto editKey = "group.transform." + juce::String(axisIndex) +
                               ":" + juce::String(groupId);
          auto apply = [this, axisIndex,
                        editKey](const juce::var &nextValue) -> bool {
            ...
            const auto applied =
                axisIndex >= 2 ? std::max(1.0f, *numeric) : *numeric;
            if (axisIndex == 0)
              target.setX(applied);
            else if (axisIndex == 1)
              target.setY(applied);
            else if (axisIndex == 2)
              target.setWidth(applied);
            else
              target.setHeight(applied);
  ...
  auto applyGroupOpacity = [this, groupId = group.id, groupOpacityKey](
                               const juce::var &nextValue) -> bool {
    const auto numeric = toFloat(nextValue);
  ...
  addEditorRow(
      opacitySpec, opacityState,
      [applyGroupOpacity](const juce::var &nextValue) {
        applyGroupOpacity(nextValue);
      },
...
void PropertyPanel::buildWidgetContent(const std::vector<WidgetRef> &widgets,
                                       bool multiSelection) {
  if (widgets.empty()) {
    buildNoneContent();
    return;
  ...
  const auto ids = inspectorTarget.widgetIds.empty()
                       ? std::vector<WidgetId>{widgets.front().id}
                       : inspectorTarget.widgetIds;
  ...
  const auto addWidgetBool =
      [this, ids](const juce::String &label, const ValueState &state,
                  const juce::String &keyPrefix,
  ...
  addWidgetBool(
      "Visible", makeBooleanState(visibleValues), "widget.visible",
      [](WidgetPropsPatch &patch, bool value) { patch.visible = value; });
  addWidgetBool(
      "Locked", makeBooleanState(lockedValues), "widget.locked",
      [](WidgetPropsPatch &patch, bool value) { patch.locked = value; });
  addSectionHeader("Transform");
  if (const auto transform = resolveCurrentTransformBounds();
      transform.has_value()) {
    const auto addTransformField = [this, ids](const juce::String &label,
                                               int axisIndex, float current) {
      Widgets::WidgetPropertySpec spec;
      ...
      const auto editKey = "widget.transform." + juce::String(axisIndex) + ":" +
                           juce::String(ids.front());
      auto apply = [this, axisIndex,
                    editKey](const juce::var &nextValue) -> bool {
        const auto numeric = toFloat(nextValue);
        const auto currentBounds = resolveCurrentTransformBounds();
  ...
  auto applyWidgetOpacity =
      [this, ids, widgetOpacityKey](const juce::var &nextValue) -> bool {
    const auto numeric = toFloat(nextValue);
    if (!numeric.has_value())
      return false;
  ...
  addEditorRow(
      opacitySpec, makeFloatState(opacityValues),
      [applyWidgetOpacity](const juce::var &nextValue) {
        applyWidgetOpacity(nextValue);
      },
...
void PropertyPanel::buildWidgetBindingSummary(
    const std::vector<WidgetRef> &widgets) {
  ...
  const auto validateBinding =
      [this, &runtimeParamDefaults,
       &isIdentifierLike](const PropertyBindingModel &binding) -> juce::String {
    ...
    const auto *spec =
        widgetFactory.propertySpecFor(widgetIt->type, propertyKey);
    ...
    const auto eval = Runtime::PropertyBindingResolver::evaluateExpression(
  ...
  addInfoRow("Count", juce::String(static_cast<int>(matchedBindings.size())) +
  ...
  for (size_t i = 0; i < previewCount; ++i) {
    ...
    const auto state = errorText.isNotEmpty()
                           ? "[ERR] "
                           : (binding->enabled ? "[ON] " : "[OFF] ");
    auto value = state + binding->targetProperty.trim() + " <- " +
  ...
  if (matchedBindings.size() > previewCount)
    addInfoRow("More", juce::String(static_cast<int>(matchedBindings.size() -
                                                     previewCount)) +
                           " additional bindings");
}

void PropertyPanel::buildCommonWidgetProperties(
    const std::vector<WidgetRef> &widgets) {
  ...
  const auto hasAdvanced = std::any_of(
      specs.begin(), specs.end(),
      [](const Widgets::WidgetPropertySpec &spec) { return spec.advanced; });
  showAdvancedToggle.setVisible(false);
  ...
  enum class SliderValueLayout { notSlider, single, range, three, mixed };
  ...
  const auto resolveSliderLayout = [&widgets]() -> SliderValueLayout {
    ...
    const auto styleToLayout =
        [](const juce::String &styleKey) -> SliderValueLayout {
      if (styleKey == "twoValueHorizontal" || styleKey == "twoValueVertical")
        return SliderValueLayout::range;
      if (styleKey == "threeValueHorizontal" ||
  ...
  const auto renderSpec =
      [this, &widgets,
       &ids](const Widgets::WidgetPropertySpec &specForRendering) {
        auto specForEditor = specForRendering;
        const auto valueState =
            makeWidgetPropertyState(widgets, specForRendering.key);
        ...
        if (specForEditor.kind == Widgets::WidgetPropertyKind::assetRef) {
          ...
          const auto currentRef = valueState.valid
                                      ? valueState.value.toString().trim()
                                      : juce::String();
          ...
          for (const auto &asset : assetList) {
            ...
            if (!Widgets::isAssetKindAccepted(specForEditor, asset.kind)) {
              if (currentRef.isNotEmpty() &&
                  refKey.equalsIgnoreCase(currentRef))
            ...
            option.value = refKey;
            option.label = asset.name.isNotEmpty()
                               ? asset.name + " (" + refKey + ")"
                               : refKey;
          ...
          if (currentRef.isNotEmpty() &&
              seenRefKeys.count(currentRefNormalized) == 0) {
        ...
        const auto editKey =
            "widget.prop." +
            specForRendering.key.toString().replaceCharacter('.', '_') + ":" +
            juce::String(ids.front());

        auto applyProp = [this, ids, specForEditor,
                          propertyKey = specForRendering.key,
                          editKey](const juce::var &nextValue) -> bool {
          juce::var normalized;
          if (!PropertyEditorFactory::normalizeValue(specForEditor, nextValue,
                                                     normalized))
            return false;
  ...
  for (const auto &spec : specs) {
    if (sliderLayout != SliderValueLayout::notSlider &&
        sliderLayout != SliderValueLayout::mixed) {
  ...
  if (hasAdvanced) {
    addExpanderRow("Advanced (" +
                       juce::String(static_cast<int>(advancedSpecs.size())) +
                       ")",
                   showAdvancedProperties, [this] {
...
void PropertyPanel::rebuildContent() {
  ...
  case InspectorTargetKind::layer:
    if (const auto *layer = findLayer(inspectorTarget.nodeId);
        layer != nullptr) {
      titleLabel.setText("Layer", juce::dontSendNotification);
      subtitleLabel.setText(layer->name + "  #" + juce::String(layer->id),
  ...
  case InspectorTargetKind::group:
    if (const auto *group = findGroup(inspectorTarget.nodeId);
        group != nullptr) {
      titleLabel.setText("Group", juce::dontSendNotification);
      subtitleLabel.setText(group->name + "  #" + juce::String(group->id),
                            juce::dontSendNotification);
  ...
  case InspectorTargetKind::widgetMulti: {
    ...
    const auto multi =
        inspectorTarget.kind == InspectorTargetKind::widgetMulti ||
        widgets.size() > 1;
    titleLabel.setText(multi ? "Widgets" : "Widget",
                       juce::dontSendNotification);
    subtitleLabel.setText(
        multi ? juce::String(static_cast<int>(widgets.size())) + " selected"
              : (widgetTypeLabel(widgetFactory, widgets.front().type) + "  #" +
                 juce::String(widgets.front().id)),
        juce::dontSendNotification);
