...
const char *dropPlacementLabel(
    Gyeol::Ui::Interaction::LayerDropPlacement placement) noexcept {
  switch (placement) {
  case Gyeol::Ui::Interaction::LayerDropPlacement::before:
    return "before";
  case Gyeol::Ui::Interaction::LayerDropPlacement::after:
    return "after";
  case Gyeol::Ui::Interaction::LayerDropPlacement::into:
    return "into";
  }
...
juce::String widgetTypeLabel(const Gyeol::Widgets::WidgetFactory &widgetFactory,
                             Gyeol::WidgetType type) {
  if (const auto *descriptor = widgetFactory.descriptorFor(type);
      descriptor != nullptr) {
...
const Gyeol::GroupModel *findGroupById(const Gyeol::DocumentModel &document,
                                       Gyeol::WidgetId groupId) noexcept {
  const auto it = std::find_if(document.groups.begin(), document.groups.end(),
...
const Gyeol::WidgetModel *findWidgetById(const Gyeol::DocumentModel &document,
                                         Gyeol::WidgetId widgetId) noexcept {
  const auto it = std::find_if(document.widgets.begin(), document.widgets.end(),
...
const Gyeol::LayerModel *findLayerById(const Gyeol::DocumentModel &document,
                                       Gyeol::WidgetId layerId) noexcept {
  const auto it = std::find_if(document.layers.begin(), document.layers.end(),
...
std::unordered_map<Gyeol::WidgetId, Gyeol::WidgetId>
directOwnerByWidgetId(const Gyeol::DocumentModel &document) {
...
std::unordered_map<Gyeol::WidgetId, int>
widgetOrderById(const Gyeol::DocumentModel &document) {
...
int groupAnchorOrder(
    const Gyeol::DocumentModel &document, Gyeol::WidgetId groupId,
  ...
  if (const auto *group = findGroupById(document, groupId); group != nullptr) {
    ...
    for (const auto &child : document.groups) {
      if (child.parentGroupId.value_or(Gyeol::kRootId) != groupId)
        continue;
      anchor = std::min(anchor, groupAnchorOrder(document, child.id,
                                                 widgetOrder, memo, visiting));
    }
  }
...
void collectGroupWidgetsRecursive(
    const Gyeol::DocumentModel &document, Gyeol::WidgetId groupId,
...
public:
  TreeItem(LayerTreePanel &ownerIn,
           const LayerTreePanel::ModelNode &modelNodeIn)
  ...
  int getItemHeight() const override { return 28; } // Increased line height
  ...
  void paintItem(juce::Graphics &g, int width, int height) override {
    ...
    const auto iconHeight = juce::jmax(14, height - 8);
    const auto visibleBounds = juce::Rectangle<float>(
        juce::jmax(0.0f, width - 38.0f), (height - iconHeight) / 2.0f, 16.0f,
        static_cast<float>(iconHeight));
    const auto lockedBounds = juce::Rectangle<float>(
        juce::jmax(0.0f, width - 20.0f), (height - iconHeight) / 2.0f, 16.0f,
        static_cast<float>(iconHeight));
    ...
    eyePath.startNewSubPath(visibleBounds.getX() + 2.0f,
                            visibleBounds.getCentreY());
    eyePath.quadraticTo(visibleBounds.getCentreX(), visibleBounds.getY() + 3.0f,
                        visibleBounds.getRight() - 2.0f,
                        visibleBounds.getCentreY());
    eyePath.quadraticTo(
        visibleBounds.getCentreX(), visibleBounds.getBottom() - 3.0f,
        visibleBounds.getX() + 2.0f, visibleBounds.getCentreY());
    g.strokePath(eyePath, juce::PathStrokeType(1.0f));
    if (modelNode.visible) {
      g.fillEllipse(visibleBounds.getCentreX() - 1.5f,
                    visibleBounds.getCentreY() - 1.5f, 3.0f, 3.0f);
    } else {
      g.drawLine(visibleBounds.getX() + 3.0f, visibleBounds.getBottom() - 3.0f,
                 visibleBounds.getRight() - 3.0f, visibleBounds.getY() + 3.0f,
                 1.2f);
    }
    ...
    if (modelNode.locked) {
      ...
      lockBody.addRoundedRectangle(lockedBounds.getCentreX() - 3.5f,
                                   lockedBounds.getCentreY() - 0.5f, 7.0f, 6.0f,
                                   1.0f);
      g.fillPath(lockBody);

      juce::Path shackle;
      shackle.addArc(lockedBounds.getCentreX() - 2.5f,
                     lockedBounds.getY() + 3.0f, 5.0f, 5.0f,
                     -juce::MathConstants<float>::halfPi - 0.2f,
                     juce::MathConstants<float>::halfPi + 0.2f, true);
      g.strokePath(shackle, juce::PathStrokeType(1.2f));
    } else {
      g.fillEllipse(lockedBounds.getCentreX() - 1.5f,
                    lockedBounds.getCentreY() - 1.5f, 3.0f, 3.0f);
    }
  }
  ...
  void itemOpennessChanged(bool isNowOpen) override {
    if (modelNode.kind == ModelNodeKind::layer) {
      if (isNowOpen)
        owner.expandedLayerIds.insert(modelNode.id);
      else
        owner.expandedLayerIds.erase(modelNode.id);
      return;
    }
...
public:
  RootItem(LayerTreePanel &owner,
           const std::vector<LayerTreePanel::ModelNode> &nodes) {
...
void LayerTreePanel::refreshFromDocument() {
  ...
  const auto selectedKey =
      resolveSelectedNodeKey(document.editorState().selection);
  if (!rebuiltTree &&
      (selectionMapChanged || previousSelectedKey != selectedKey)) {
  ...
  perf.refreshCount += 1;
  perf.lastRebuildModelMs = std::chrono::duration<double, std::milli>(
                                rebuildModelEnd - rebuildModelStart)
                                .count();
  perf.lastRebuildTreeMs = std::chrono::duration<double, std::milli>(
                               rebuildTreeEnd - rebuildModelEnd)
                               .count();
  perf.lastRefreshMs =
  ...
  const auto shouldLog = perf.lastRefreshMs >= slowRefreshLogThresholdMs ||
                         (periodicPerfLogInterval > 0 &&
                          perf.refreshCount % periodicPerfLogInterval == 0);
  if (shouldLog) {
    DBG(juce::String("[Gyeol][LayerTreePanel][Perf] refresh#") +
        ...
        " dragUpdatesSinceRefresh=" +
        juce::String(static_cast<int64_t>(dragUpdatesSinceRefresh)) +
        " dragUpdatesTotal=" +
...
void LayerTreePanel::setSelectionChangedCallback(
    std::function<void(std::vector<WidgetId>)> callback) {
  onSelectionChanged = std::move(callback);
}

std::optional<NodeRef> LayerTreePanel::selectedNode() const {
  if (const auto *selected =
          dynamic_cast<TreeItem *>(treeView.getSelectedItem(0));
      selected != nullptr)
    return NodeRef{nodeKindFromModelKind(selected->node().kind),
                   selected->node().id};

  return std::nullopt;
}

void LayerTreePanel::setActiveLayerChangedCallback(
    std::function<void(std::optional<WidgetId>)> callback) {
  onActiveLayerChanged = std::move(callback);
}

void LayerTreePanel::setDropRequestCallback(
    std::function<juce::Result(const Interaction::LayerTreeDropRequest &)>
        callback) {
  onDropRequest = std::move(callback);
}

void LayerTreePanel::setNodePropsChangedCallback(
    std::function<juce::Result(const SetPropsAction &)> callback) {
  onNodePropsChanged = std::move(callback);
}

void LayerTreePanel::setCreateLayerRequestedCallback(
    std::function<std::optional<WidgetId>()> callback) {
  onCreateLayerRequested = std::move(callback);
}

void LayerTreePanel::setDeleteLayerRequestedCallback(
    std::function<juce::Result(WidgetId)> callback) {
  onDeleteLayerRequested = std::move(callback);
}

const char *LayerTreePanel::dragKindToken(ModelNodeKind kind) noexcept {
  switch (kind) {
  case ModelNodeKind::layer:
    return "layer";
  case ModelNodeKind::group:
    return "group";
  case ModelNodeKind::widget:
    return "widget";
  }
...
std::optional<LayerTreePanel::ModelNodeKind>
LayerTreePanel::modelKindFromDragToken(const juce::String &token) {
  if (token == "layer")
...
juce::var
LayerTreePanel::buildDragDescription(ModelNodeKind kind,
...
void LayerTreePanel::paintOverChildren(juce::Graphics &g) {
  ...
  if (dropPreview->placement == Interaction::LayerDropPlacement::into) {
    ...
    g.setColour(accent);
    g.drawRoundedRectangle(marker.toFloat().reduced(1.0f), 6.0f, 1.5f);
  } else {
    g.setColour(accent);
    g.fillRect(marker);
  }
...
void LayerTreePanel::mouseDown(const juce::MouseEvent &event) {
  if (!event.mods.isLeftButtonDown())
    return;
  if (!treeView.isParentOf(event.eventComponent) &&
      event.eventComponent != &treeView)
    return;
  ...
  for (auto *item : visibleTreeItems()) {
    ...
    if (iconHitForTreeItem(*item, local) != RowIcon::none) {
      logLayerTreeDnd("mouseDown ignored: icon click node=" +
                      keyForNode(item->node().kind, item->node().id));
      return;
    }
...
void LayerTreePanel::mouseUp(const juce::MouseEvent &) { endDrag(); }

bool LayerTreePanel::isInterestedInDragSource(
    const juce::DragAndDropTarget::SourceDetails &dragSourceDetails) {
  ModelNodeKind kind = ModelNodeKind::widget;
  std::vector<WidgetId> ids;
  const auto ok =
      parseDragDescription(dragSourceDetails.description, kind, ids) &&
      !ids.empty();
  logLayerTreeDnd("isInterestedInDragSource=" +
                  juce::String(ok ? "true" : "false"));
  return ok;
}

void LayerTreePanel::itemDragEnter(
    const juce::DragAndDropTarget::SourceDetails &dragSourceDetails) {
  itemDragMove(dragSourceDetails);
}

void LayerTreePanel::itemDragMove(
    const juce::DragAndDropTarget::SourceDetails &dragSourceDetails) {
  ModelNodeKind kind = ModelNodeKind::widget;
  std::vector<WidgetId> ids;
  if (!parseDragDescription(dragSourceDetails.description, kind, ids) ||
      ids.empty()) {
  ...
  const auto treePoint =
      dragSourceDetails.localPosition.toInt() - treeView.getPosition();
  handleDragMoveAt(treePoint);
}

void LayerTreePanel::itemDragExit(
...
void LayerTreePanel::itemDropped(
    const juce::DragAndDropTarget::SourceDetails &dragSourceDetails) {
  ModelNodeKind kind = ModelNodeKind::widget;
  std::vector<WidgetId> ids;
  if (!parseDragDescription(dragSourceDetails.description, kind, ids) ||
...
std::vector<LayerTreePanel::ModelNode>
LayerTreePanel::buildNodesForParent(const ParentRef &parent) const {
  const auto &snapshot = document.snapshot();
  ...
  if (parent.kind == ParentKind::layer) {
    ...
    std::unordered_set<WidgetId> layerGroups(layer->memberGroupIds.begin(),
                                             layer->memberGroupIds.end());
    std::unordered_set<WidgetId> layerWidgets(layer->memberWidgetIds.begin(),
    ...
    for (const auto &group : snapshot.groups) {
      ...
      const auto anchor = groupAnchorOrder(snapshot, group.id, orderByWidgetId,
                                           groupAnchorMemo, visiting);
      candidates.push_back({true, group.id, anchor});
    }

    for (const auto &widget : snapshot.widgets) {
      const auto owner = ownerByWidgetId.count(widget.id) > 0
                             ? ownerByWidgetId.at(widget.id)
                             : kRootId;
  ...
  for (auto it = candidates.rbegin(); it != candidates.rend(); ++it) {
    ...
    if (it->isGroup) {
      ...
      node.label =
          group->name.isNotEmpty() ? group->name : juce::String("Group");
...
std::vector<LayerTreePanel::ModelNode> LayerTreePanel::filterNodes(
    const std::vector<LayerTreePanel::ModelNode> &source,
    const juce::String &filterLower) const {
...
bool LayerTreePanel::modelNodeMatchesFilter(
    const LayerTreePanel::ModelNode &node,
    const juce::String &filterLower) const {
  return filterLower.isEmpty() || node.filterKeyLower.contains(filterLower);
}

std::vector<WidgetId>
LayerTreePanel::collectGroupSelectionIds(WidgetId groupId) const {
...
std::optional<LayerTreePanel::ModelNode> LayerTreePanel::findNodeByKey(
    const juce::String &key,
    const std::vector<LayerTreePanel::ModelNode> &nodes) const {
  std::function<std::optional<LayerTreePanel::ModelNode>(
      const std::vector<LayerTreePanel::ModelNode> &)>
      findRecursive =
          [&](const std::vector<LayerTreePanel::ModelNode> &currentNodes)
      -> std::optional<LayerTreePanel::ModelNode> {
...
std::optional<LayerTreePanel::ModelNode>
LayerTreePanel::findNodeByKey(const juce::String &key) const {
  return findNodeByKey(key, fullRootNodes);
}

std::optional<juce::String> LayerTreePanel::resolveSelectedNodeKey(
    const std::vector<WidgetId> &selection) const {
  if (selection.empty()) {
...
void LayerTreePanel::rebuildModel() {
  ...
  std::function<void(const std::vector<LayerTreePanel::ModelNode> &)>
      collectIds = [&](const std::vector<LayerTreePanel::ModelNode> &nodes) {
        for (const auto &node : nodes) {
  ...
  if (explicitActiveLayerId.has_value() &&
      currentLayerIds.count(*explicitActiveLayerId) == 0)
  ...
  std::function<void(const std::vector<LayerTreePanel::ModelNode> &)>
      indexNodes = [&](const std::vector<LayerTreePanel::ModelNode> &nodes) {
        for (const auto &node : nodes) {
...
void LayerTreePanel::applyDocumentSelectionToTree() {
  ...
  if (lastAppliedSelectionKey.has_value()) {
    if (auto *previousItem = findVisibleTreeItemByKey(*lastAppliedSelectionKey);
  ...
  if (key.has_value()) {
    if (auto *nextItem = findTreeItemByKey(*key); nextItem != nullptr) {
      for (auto *parent = nextItem->getParentItem(); parent != nullptr;
...
void LayerTreePanel::rebuildTree() {
  ...
  std::function<void(juce::TreeViewItem *)> applyOpen =
        ...
        for (int i = 0; i < item->getNumSubItems(); ++i) {
          ...
          if (auto *treeItem = dynamic_cast<TreeItem *>(subItem)) {
            if (treeItem->node().kind == ModelNodeKind::layer)
              treeItem->setOpen(expandedLayerIds.count(treeItem->node().id) >
                                0);
            if (treeItem->node().kind == ModelNodeKind::group)
              treeItem->setOpen(expandedGroupIds.count(treeItem->node().id) >
                                0);
          }
...
void LayerTreePanel::beginDrag(ModelNodeKind kind, WidgetId id,
                               juce::Point<int> treePoint) {
  dragCandidate = true;
  dragActive = false;
  dragNodeKind = kind;
  draggedNodeIds = {id};
  ...
  logLayerTreeDnd("beginDrag kind=" + juce::String(dragKindToken(kind)) +
                  " id=" + juce::String(id) + " start=(" +
                  juce::String(treePoint.x) + "," + juce::String(treePoint.y) +
                  ")");
}
...
void LayerTreePanel::updateDrag(juce::Point<int> treePoint) {
  ...
  if (!dragActive) {
    if ((treePoint - dragStartPoint).getDistanceFromOrigin() < 4)
      return;
    dragActive = true;
    logLayerTreeDnd("updateDrag activated kind=" +
                    juce::String(dragKindToken(dragNodeKind)) +
...
void LayerTreePanel::handleDragMoveAt(juce::Point<int> treePoint) {
  ...
  if (hasChanged) {
    if (dropPreview.has_value()) {
      logLayerTreeDnd(
          "preview target=" + dropPreview->targetNodeKey + " placement=" +
          dropPlacementLabel(dropPreview->placement) + " point=(" +
          juce::String(treePoint.x) + "," + juce::String(treePoint.y) + ")");
    } else {
      logLayerTreeDnd("preview cleared point=(" + juce::String(treePoint.x) +
                      "," + juce::String(treePoint.y) + ")");
    }
  }
}
...
void LayerTreePanel::endDrag() {
  ...
  logLayerTreeDnd(
      "endDrag active=" + juce::String(dragActive ? "true" : "false") +
      " ids=" + widgetIdsToDebugString(draggedNodeIds) + " hasPreview=" +
      juce::String(dropPreview.has_value() ? "true" : "false"));
  ...
  if (dragActive && dropPreview.has_value() && onDropRequest != nullptr) {
    if (auto request = buildDropRequest(*dropPreview); request.has_value()) {
      logLayerTreeDnd(
          "commitDrop parent=" + parentRefToDebugString(request->parent) +
...
void LayerTreePanel::repaintDropPreviewDiff(
    const std::optional<DropPreview> &previousPreview,
    const std::optional<DropPreview> &nextPreview) {
  juce::Rectangle<int> dirty;
  auto includePreview = [this,
                         &dirty](const std::optional<DropPreview> &preview) {
    if (!preview.has_value())
      return;
    const auto marker =
        preview->markerBounds.translated(treeView.getX(), treeView.getY())
            .expanded(3);
...
void LayerTreePanel::updateAutoExpand(
    const std::optional<DropPreview> &preview) {
  if (!preview.has_value() ||
...
void LayerTreePanel::maybeAutoScroll(juce::Point<int> treePoint) {
  ...
  for (int i = 0; i < treeView.getNumChildComponents(); ++i) {
    if (auto *childViewport =
            dynamic_cast<juce::Viewport *>(treeView.getChildComponent(i));
        childViewport != nullptr) {
  ...
  const auto nextY =
      juce::jlimit(0, maxY, viewport->getViewPositionY() + deltaY);
...
std::optional<LayerTreePanel::DropPreview>
LayerTreePanel::computeDropPreview(juce::Point<int> treePoint) const {
  ...
  if (target == nullptr) {
    target = *std::min_element(
        ...
        [this, treePoint](const TreeItem *lhs, const TreeItem *rhs) {
  ...
  if (dragNodeKind == ModelNodeKind::layer &&
      target->node().kind != ModelNodeKind::layer)
    return std::nullopt;
  ...
  // Non-layer drags should not resolve "before/after" against layer header
  // rows. For layer headers, force a deterministic "into layer" preview.
  if (dragNodeKind != ModelNodeKind::layer &&
      target->node().kind == ModelNodeKind::layer) {
  ...
  if (treePoint.y <= upperZone) {
    ...
    preview.markerBounds = {row.getX() + 4, row.getY() - 1,
                            std::max(8, row.getWidth() - 8), 2};
  } else if (treePoint.y >= lowerZone) {
    preview.placement = Interaction::LayerDropPlacement::after;
    preview.markerBounds = {row.getX() + 4, row.getBottom() - 1,
                            std::max(8, row.getWidth() - 8), 2};
    ...
    const auto markerY =
        preview.placement == Interaction::LayerDropPlacement::before
            ? row.getY()
            : row.getBottom();
    preview.markerBounds = {row.getX() + 4, markerY - 1,
                            std::max(8, row.getWidth() - 8), 2};
  }
...
std::optional<Interaction::LayerTreeDropRequest>
LayerTreePanel::buildDropRequest(const DropPreview &preview) const {
  const auto parentAndInsert = resolveDropParentAndInsert(preview);
  if (!parentAndInsert.has_value()) {
    logLayerTreeDnd(
        "buildDropRequest failed: cannot resolve parent/insert for target=" +
        preview.targetNodeKey);
    return std::nullopt;
  }
  ...
  if (const auto targetNode =
          findNodeByKey(preview.targetNodeKey, fullRootNodes);
      targetNode.has_value())
    request.target =
        NodeRef{nodeKindFromModelKind(targetNode->kind), targetNode->id};

  logLayerTreeDnd("buildDropRequest ok draggedKind=" +
                  juce::String(dragKindToken(dragNodeKind)) +
...
std::optional<std::pair<ParentRef, int>>
LayerTreePanel::resolveDropParentAndInsert(const DropPreview &preview) const {
  ...
  if (preview.placement == Interaction::LayerDropPlacement::into) {
    if (targetNode->kind == ModelNodeKind::widget)
      return std::nullopt;
    ParentRef parent;
    parent.kind = targetNode->kind == ModelNodeKind::layer ? ParentKind::layer
                                                           : ParentKind::group;
  ...
  if (parent.kind == ParentKind::root) {
    ...
    const auto parentKind = parent.kind == ParentKind::layer
                                ? ModelNodeKind::layer
                                : ModelNodeKind::group;
    parentNodeStorage =
        findNodeByKey(keyForNode(parentKind, parent.id), fullRootNodes);
    if (!parentNodeStorage.has_value())
      return std::nullopt;
    siblings = &parentNodeStorage->children;
  }

  std::unordered_set<WidgetId> draggedSet(draggedNodeIds.begin(),
                                          draggedNodeIds.end());
  std::vector<LayerTreePanel::ModelNode> filtered;
  ...
  const auto targetFrontIndex =
      static_cast<int>(std::distance(filtered.begin(), targetIt));
  const auto siblingCount = static_cast<int>(filtered.size());
  // Reducer insertIndex is back-to-front (0=back, end=front), while Tree UI is
  // front-first.
...
bool LayerTreePanel::isDropTargetInDraggedSubtree(
    const juce::String &targetNodeKey) const {
  ...
  if (dragNodeKind == ModelNodeKind::widget)
    return targetNode->kind == ModelNodeKind::widget &&
           std::find(draggedNodeIds.begin(), draggedNodeIds.end(),
                     targetNode->id) != draggedNodeIds.end();
  ...
  for (const auto draggedGroupId : draggedNodeIds) {
    ...
    while (parent.kind == ParentKind::group) {
      ...
      const auto parentNode = findNodeByKey(
          keyForNode(ModelNodeKind::group, parent.id), fullRootNodes);
...
juce::Rectangle<int>
LayerTreePanel::rowBoundsForItem(const TreeItem &item) const {
...
std::vector<LayerTreePanel::TreeItem *>
LayerTreePanel::visibleTreeItems() const {
  std::vector<TreeItem *> items;
  collectVisibleTreeItems(rootItem.get(), items);
  return items;
}

void LayerTreePanel::collectVisibleTreeItems(juce::TreeViewItem *item,
...
bool LayerTreePanel::sameWidgetIdSet(const std::vector<WidgetId> &lhs,
                                     const std::vector<WidgetId> &rhs) {
...
LayerTreePanel::RowIcon
LayerTreePanel::iconHitForTreeItem(const TreeItem &item,
                                   juce::Point<int> localPoint) const {
  if (item.paintWidth() <= 0)
    return RowIcon::none;

  const auto h = juce::jmax(12, item.getItemHeight() - 6);
  const auto visibleBounds =
      juce::Rectangle<int>(juce::jmax(0, item.paintWidth() - 36), 3, 14, h);
  const auto lockedBounds =
      juce::Rectangle<int>(juce::jmax(0, item.paintWidth() - 18), 3, 14, h);
...
void LayerTreePanel::toggleNodeIcon(const LayerTreePanel::ModelNode &node,
                                    RowIcon icon) {
  if (icon == RowIcon::none || onNodePropsChanged == nullptr)
    return;

  const auto nextValue =
      icon == RowIcon::visible ? !node.visible : !node.locked;
  ...
  if (action.kind == NodeKind::widget) {
    ...
    if (icon == RowIcon::visible)
      patch.visible = nextValue;
    else
      patch.locked = nextValue;
    action.patch = patch;
...
void LayerTreePanel::handleDeleteLayerButton() {
  ...
  if (result.failed())
    DBG("[Gyeol] LayerTreePanel delete layer failed: " +
        result.getErrorMessage());
}

bool LayerTreePanel::sameNodeStructure(const std::vector<ModelNode> &lhs,
                                       const std::vector<ModelNode> &rhs) {
  if (lhs.size() != rhs.size())
    return false;
  ...
  for (size_t i = 0; i < lhs.size(); ++i) {
    ...
    if (leftNode.kind != rightNode.kind || leftNode.id != rightNode.id)
      return false;
    if (leftNode.parent.kind != rightNode.parent.kind ||
        leftNode.parent.id != rightNode.parent.id)
      return false;
...
bool LayerTreePanel::sameNodeVisuals(const std::vector<ModelNode> &lhs,
                                     const std::vector<ModelNode> &rhs) {
  if (lhs.size() != rhs.size())
    return false;
  ...
  for (size_t i = 0; i < lhs.size(); ++i) {
    ...
    if (leftNode.label != rightNode.label)
      return false;
    if (leftNode.visible != rightNode.visible ||
        leftNode.locked != rightNode.locked)
      return false;
...
void LayerTreePanel::collectVisualChangedKeys(
    const std::vector<ModelNode> &previousNodes,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    collectVisualChangedKeys(previousNode.children, nextNode.children,
                             outChangedKeys);
  }
}
...
LayerTreePanel::TreeItem *
LayerTreePanel::findTreeItemByKey(const juce::String &key) const {
  if (rootItem == nullptr || key.isEmpty())
    return nullptr;

  std::function<TreeItem *(juce::TreeViewItem *)> findRecursive =
      [&](juce::TreeViewItem *item) -> TreeItem * {
    if (item == nullptr)
      return nullptr;
...
LayerTreePanel::TreeItem *
LayerTreePanel::findVisibleTreeItemByKey(const juce::String &key) const {
  for (auto *item : visibleTreeItems()) {
...
const char *
LayerTreePanel::refreshReasonToString(RefreshReason reason) noexcept {
  switch (reason) {
  case RefreshReason::external:
    return "external";
  case RefreshReason::initial:
    return "initial";
  case RefreshReason::searchChanged:
    return "search";
  }
  return "unknown";
}
