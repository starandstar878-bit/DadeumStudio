...
bool parseCsvDoubles(const juce::String &text, std::vector<double> &outValues,
                     int expectedCount) {
  outValues.clear();
...
bool validateNumericRange(const Widgets::WidgetPropertySpec &spec,
                          double value) {
...
bool readObjectNumber(const juce::var &input, const juce::Identifier &key,
                      double &outValue) {
...
std::optional<AssetDropPayload>
extractAssetDropPayloadFromDragDescription(const juce::var &description) {
  ...
  const auto refKey =
      props.getWithDefault("refKey", juce::var()).toString().trim();
  if (refKey.isEmpty())
    return std::nullopt;
  ...
  if (const auto parsedKind = assetKindFromKey(
          props.getWithDefault("assetKind", juce::var()).toString().trim());
...
inferAssetKindFromDropPayload(const AssetDropPayload &payload) {
...
bool isAssetDropPayloadAccepted(const Widgets::WidgetPropertySpec &spec,
                                const AssetDropPayload &payload) {
...
public:
  ...
  bool isInterestedInDragSource(const juce::DragAndDropTarget::SourceDetails
    ...
    return extractAssetDropPayloadFromDragDescription(
  ...
  void itemDropped(const juce::DragAndDropTarget::SourceDetails
                       &dragSourceDetails) override {
    const auto payload = extractAssetDropPayloadFromDragDescription(
        dragSourceDetails.description);
...
private:
  void updateDragFeedback(
    ...
    dragAllowed =
        dragHovering && (isDropAllowed == nullptr || isDropAllowed(*payload));
    repaint();
  }
...
createTextEditor(const EditorBuildSpec &buildSpec,
                 const juce::String &initialText,
                 const juce::String &placeholder) {
  auto editor = std::make_unique<juce::TextEditor>();
  const auto isMultiline =
      buildSpec.spec.uiHint == Widgets::WidgetPropertyUiHint::multiLine;
  ...
  editor->onTextChange =
      [safe = juce::Component::SafePointer<juce::TextEditor>(editor.get()),
       invokePreview, state] {
        if (safe == nullptr)
          return;
...
juce::String
PropertyEditorFactory::formatValue(const Widgets::WidgetPropertySpec &spec,
...
bool PropertyEditorFactory::parseValue(const Widgets::WidgetPropertySpec &spec,
  ...
  case Widgets::WidgetPropertyKind::integer: {
    ...
    if (spec.minValue.has_value() &&
        static_cast<double>(parsed) < *spec.minValue)
      return false;
    if (spec.maxValue.has_value() &&
        static_cast<double>(parsed) > *spec.maxValue)
      return false;
  ...
  case Widgets::WidgetPropertyKind::enumChoice: {
    const auto it = std::find_if(
        spec.enumOptions.begin(), spec.enumOptions.end(),
...
bool PropertyEditorFactory::normalizeValue(
    const Widgets::WidgetPropertySpec &spec, const juce::var &input,
    juce::var &valueOut) {
  if (input.isString())
  ...
  case Widgets::WidgetPropertyKind::enumChoice: {
    ...
    const auto it =
        std::find_if(spec.enumOptions.begin(), spec.enumOptions.end(),
  ...
  case Widgets::WidgetPropertyKind::color: {
    ...
    if (spec.colorStorage == Widgets::ColorStorage::rgbaObject255 ||
      ...
      double g = 0.0;
      double b = 0.0;
      double a = spec.colorStorage == Widgets::ColorStorage::rgbaObject255
                     ? 255.0
                     : 1.0;
      ...
      const auto min = 0.0;
      const auto max = spec.colorStorage == Widgets::ColorStorage::rgbaObject255
                           ? 255.0
                           : 1.0;
      if (r < min || g < min || b < min || a < min || r > max || g > max ||
          b > max || a > max)
        return false;
  ...
  case Widgets::WidgetPropertyKind::rect: {
    ...
    double h = 0.0;
    if (!readObjectNumber(input, "x", x) || !readObjectNumber(input, "y", y) ||
        !readObjectNumber(input, "w", w) || !readObjectNumber(input, "h", h)) {
      return false;
    }
    if (w < 0.0 || h < 0.0)
      return false;
...
std::unique_ptr<juce::Component>
PropertyEditorFactory::createEditor(const EditorBuildSpec &buildSpec) {
  ...
  if (buildSpec.spec.kind == Widgets::WidgetPropertyKind::boolean) {
    ...
    editor->setToggleState(buildSpec.mixed ? false
                                           : static_cast<bool>(buildSpec.value),
                           juce::dontSendNotification);
    editor->setButtonText(buildSpec.mixed ? juce::String("Mixed")
                                          : juce::String());
    editor->setEnabled(!readOnly);

    if (!readOnly) {
      editor->onClick = [buildSpec,
                         safe =
                             juce::Component::SafePointer<juce::ToggleButton>(
                                 editor.get())] {
        if (safe == nullptr)
          return;
        juce::var normalized;
        if (!PropertyEditorFactory::normalizeValue(
                buildSpec.spec, juce::var(safe->getToggleState()), normalized))
          return;
        if (buildSpec.onPreview)
  ...
  const auto shouldUseCombo =
      (buildSpec.spec.kind == Widgets::WidgetPropertyKind::enumChoice ||
       (buildSpec.spec.kind == Widgets::WidgetPropertyKind::assetRef &&
        (buildSpec.spec.uiHint == Widgets::WidgetPropertyUiHint::dropdown ||
         buildSpec.spec.uiHint ==
             Widgets::WidgetPropertyUiHint::assetPicker))) &&
  ...
  if (shouldUseCombo) {
    ...
    editor->setEnabled(!readOnly);
    editor->setEditableText(buildSpec.spec.kind ==
                            Widgets::WidgetPropertyKind::assetRef);
    int itemId = 1;
    for (const auto &option : buildSpec.spec.enumOptions)
      editor->addItem(option.label.isNotEmpty() ? option.label : option.value,
                      itemId++);
    ...
    if (!readOnly) {
      editor->onChange =
          [buildSpec,
           safe = juce::Component::SafePointer<juce::ComboBox>(editor)] {
            if (safe == nullptr)
              return;
            ...
            if (index >= 0 &&
                index < static_cast<int>(buildSpec.spec.enumOptions.size())) {
              selectedText =
                  buildSpec.spec.enumOptions[static_cast<size_t>(index)].value;
            } else if (buildSpec.spec.kind ==
                       Widgets::WidgetPropertyKind::assetRef) {
            ...
            juce::var normalized;
            if (!PropertyEditorFactory::normalizeValue(
                    buildSpec.spec, juce::var(selectedText), normalized)) {
              return;
            }
            if (buildSpec.onPreview)
              buildSpec.onPreview(normalized);
      ...
      if (auto *assetDropCombo = dynamic_cast<AssetDropComboBox *>(editor);
          assetDropCombo != nullptr) {
        assetDropCombo->isDropAllowed =
            [spec = buildSpec.spec](const AssetDropPayload &payload) {
              return isAssetDropPayloadAccepted(spec, payload);
            };

        assetDropCombo->onAssetDropped =
            [buildSpec, safe = juce::Component::SafePointer<juce::ComboBox>(
                            editor)](const juce::String &refKey) {
              if (safe == nullptr)
                return;
              ...
              juce::var normalized;
              if (!PropertyEditorFactory::normalizeValue(
                      buildSpec.spec, juce::var(refKey), normalized))
                return;
  ...
  const auto useSpinner = !buildSpec.mixed && !readOnly && isNumeric;

  if (useSpinner) {
    auto slider = std::make_unique<juce::Slider>(juce::Slider::LinearBar,
                                                 juce::Slider::TextBoxLeft);
    auto minValue = buildSpec.spec.minValue.value_or(-1000000.0);
    auto maxValue = buildSpec.spec.maxValue.value_or(1000000.0);

    slider->setRange(
        minValue, maxValue,
        buildSpec.spec.step.value_or(
            buildSpec.spec.kind == Widgets::WidgetPropertyKind::integer
                ? 1.0
                : 0.01));
    ...
    slider->setValue(static_cast<double>(buildSpec.value),
                     juce::dontSendNotification);
    slider->setEnabled(!readOnly);

    // Allow double-click to type in number
    slider->setDoubleClickReturnValue(
        true, static_cast<double>(buildSpec.spec.defaultValue),
        juce::ModifierKeys::noModifiers);

    slider->onValueChange =
        [buildSpec,
         safe = juce::Component::SafePointer<juce::Slider>(slider.get())] {
          if (safe == nullptr)
            return;
          juce::var nextValue;
          ...
          juce::var normalized;
          if (!PropertyEditorFactory::normalizeValue(buildSpec.spec, nextValue,
                                                     normalized))
            return;
  ...
  const auto initialText = buildSpec.mixed
                               ? juce::String()
                               : formatValue(buildSpec.spec, buildSpec.value);
  return createTextEditor(buildSpec, initialText,
                          buildSpec.mixed ? juce::String("--")
                                          : juce::String());
}
} // namespace Gyeol::Ui::Panels
