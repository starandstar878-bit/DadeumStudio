...
EventActionPanel::EventActionPanel(DocumentHandle &documentIn,
  ...
  setupEditor(patchEditor,
              juce::String::fromUTF8(u8"?Ôß? JSON ???ßÏâê ??ŒΩ??(??´Î§æÎ¨?????"));
...
void EventActionPanel::rebuildWidgetOptions() {
  ...
  for (const auto &widget : snapshot.widgets) {
    ...
    // ?????Á≠åÏôñ??????Â∑?properties["name"]) ??®ÏÄ™Ìê®, ??Í≥∏Î™µÁ≠??????ÜÍµ¨ + ID
...
// ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????
// Action ?????Ë¢Å‚ëπÏ°?????ßÏâê Á≠åÎ§¥Î´ñ‰ª•??targetPropertyCombo????åÎîÑ??
// ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????
void EventActionPanel::rebuildActionTargetPropertyOptions(
  ...
  if (targetWidgetId > kRootId) {
    ...
    // ??§Î≤è?????ßÏâê (??Êπ???ÎΩ?ªª)
    addItem("visible", juce::String::fromUTF8(u8"??ÎΩ?ªª ???"));
    addItem("locked", juce::String::fromUTF8(u8"??´Îê≤?????"));
    addItem("opacity", juce::String::fromUTF8(u8"?Î∏çÎúá??µÏ≠ó??ÖÏ¶≤"));

    // ?Ë¢Å‚ëπÏ°??????á¬Ä?spec ???ßÏâê
    const auto &snapshot = document.snapshot();
    const auto wIt =
        std::find_if(snapshot.widgets.begin(), snapshot.widgets.end(),
...
// ??´Î§æÎ¨?????ßÏâê spec??Á≠åÎùø??????âÏùÖ ?ÔßèÍæ©Ï∂øÁñ´Íø∏ÌÄ? ??Î∞¥Ïâê/??Ä≈ä??
void EventActionPanel::rebuildDynamicPropEditor() {
  ...
  spec.kind = Widgets::WidgetPropertyKind::text; // ?´Íø∏???Ï≤?
  ...
  buildSpec.onPreview = nullptr; // ???∞Îªª??Ê≤ÉÏÑéÏ±?Í≥?â™??©‚îõ ?Î∏çÎúá???
...
// ???âÏùÖ ?ÔßèÍæ©Ï∂øÁñ´Íø∏ÌÄ£ËÇâ???Ë¢Å‚ëπ???∂ÏèÖ???????Ñ† ?ÑÏèÜÍº??
...
void EventActionPanel::refreshDetailEditors() {
  ...
  if (hasAction) {
    ...
    // setNodeProps: ???ßÏâê Á≠åÎ§¥Î´ñ‰ª•?????????Ë¢Å‚ëπ?????Ë≤´Ôßç????ßÏâê ??? ??´Î§æÎ¨?
    if (action->kind == RuntimeActionKind::setNodeProps) {
      // ???Ë≤´Ôßç?patch ??È§?Á≠åÔΩÉÍ∫ÇËã°?êÏ?Ôß? preferred key??????
      juce::String prefKey;
...
void EventActionPanel::updateActionEditorVisibility(
  ...
  if (isSetNodeProps) {
    ...
    // patchEditor????§Î≤è?????ßÏâê(visible, locked, opacity)???Ë¢Å‚ë§ÎπçÁ≠å???úÎòª??éÏ≥û???ÎΩ?ªª
...
void EventActionPanel::applySelectedAction() {
  ...
  case RuntimeActionKind::setNodeProps: {
    ...
    // ????Á≠åÏöä??? Á≠åÎùæ?£ÈçÆ??????? patchEditor????? ???ÖÎºÑÁ≠??´Íº≤????ªÏ¶≤ Á≠å„ÖºÎÆ???
...
juce::String EventActionPanel::formatEventLabel(
  ...
  // ?´Íø∏?????onClick ??????ßÏëµ??ëÏóê??ÔßèÍæ™???? ??Íæ™¬Ä???????ÅÏÇ¥ÎÆ??????Â∑≤„ÉØÏ≠???ÎΩ?ªª
  ...
  return eventSpec.key; // ?ïÍ≥ï?ÜËÇâ????©ÎÆâ ?éÍªã???Ï≠????Í∂?????ÎΩ?ªª
}
