...
public:
  ...
  using SetPropsPreviewCallback =
      std::function<juce::Result(const SetPropsAction &)>;
  using SetBoundsPreviewCallback =
      std::function<juce::Result(const SetBoundsAction &)>;
  using GroupTransformPreviewCallback =
      std::function<juce::Result(WidgetId, const juce::Rectangle<float> &)>;
  ...
  PropertyPanel(DocumentHandle &documentIn,
...
private:
  ...
  void addExpanderRow(const juce::String &label, bool expanded,
  ...
  ValueState makeWidgetPropertyState(const std::vector<WidgetRef> &widgets,
  ...
  computeUnionBounds(const std::vector<WidgetId> &widgetIds) const;
  ...
  bool applySetPropsPreview(const SetPropsAction &action,
                            const juce::String &editKey);
  bool applySetBoundsPreview(const SetBoundsAction &action,
                             const juce::String &editKey);
  bool applyTransformPreview(const juce::Rectangle<float> &targetBounds,
                             const juce::String &editKey);

  juce::Rectangle<float> clampWidgetBounds(const WidgetRef &widget,
                                           juce::Rectangle<float> bounds) const;
  bool
  buildScaledBoundsUpdates(const std::vector<WidgetRef> &widgets,
  ...
  void buildWidgetContent(const std::vector<WidgetRef> &widgets,
                          bool multiSelection);
  ...
  std::vector<Widgets::WidgetPropertySpec>
  commonPropertySpecs(const std::vector<WidgetRef> &widgets) const;
