...
class AssetsPanel : public juce::Component,
                    ...
                    private juce::Timer {
public:
  AssetsPanel(DocumentHandle &documentIn,
              const Widgets::WidgetFactory &widgetFactoryIn);
  ~AssetsPanel() override;

  void refreshFromDocument();
  void
  setAssetsChangedCallback(std::function<void(const juce::String &)> callback);
...
private:
  ...
  enum class ImportConflictPolicy { rename, overwrite, skip };
  ...
  void selectAssetByRow(int row);
  void selectAssetById(WidgetId id);
  ...
  juce::String makeUniqueRefKey(const juce::String &seed,
                                WidgetId ignoreAssetId = kRootId) const;
  ...
  juce::Typeface::Ptr
  getFontPreviewTypefaceForAsset(const AssetModel &asset) const;
  ...
  static juce::Result varToPropertyBag(const juce::var &value,
                                       PropertyBag &outBag);
