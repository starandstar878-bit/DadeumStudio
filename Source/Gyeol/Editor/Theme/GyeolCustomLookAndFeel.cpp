...
void GyeolCustomLookAndFeel::drawValidationItem(
    juce::Graphics &g,
    juce::Rectangle<int> bounds,
    const juce::String &severityLabel,
    ...
    bool muted) const {
  auto box = bounds.toFloat().reduced(0.5f);
  auto surface = getGyeolColor(selected ? GyeolPalette::SelectionBackground
                                        : GyeolPalette::ControlBase);
  if (hovered && !selected)
    surface = surface.interpolatedWith(getGyeolColor(GyeolPalette::ControlHover), 0.45f);

  g.setColour(surface.withAlpha(selected ? 0.95f : 0.78f));
  g.fillRoundedRectangle(box, 5.0f);

  g.setColour(getGyeolColor(GyeolPalette::BorderDefault).withAlpha(0.95f));
  g.drawRoundedRectangle(box, 5.0f, 1.0f);

  const auto pulse = juce::jlimit(0.0f, 1.0f, pulsePhase);
  const auto pulseAlpha = 0.26f + (0.38f * pulse);
  auto iconBounds = juce::Rectangle<float>(box.getX() + 8.0f, box.getY() + 8.0f, 14.0f, 14.0f);
  g.setColour(severityColour.withAlpha(pulseAlpha));
  g.fillEllipse(iconBounds.expanded(2.5f * pulse));
  g.setColour(severityColour.withAlpha(0.95f));
  g.fillEllipse(iconBounds);

  auto chipBounds = juce::Rectangle<float>(box.getX() + 26.0f, box.getY() + 7.0f, 58.0f, 14.0f);
  g.setColour(severityColour.withAlpha(0.25f));
  g.fillRoundedRectangle(chipBounds, 3.0f);
  g.setColour(severityColour.withAlpha(0.92f));
  g.setFont(makeFont(9.5f, true));
  g.drawText(severityLabel,
             chipBounds.toNearestInt(),
             juce::Justification::centred,
             true);

  auto titleBounds = juce::Rectangle<int>(
      static_cast<int>(box.getX() + 88.0f),
      static_cast<int>(box.getY() + 6.0f),
      bounds.getWidth() - 170,
      16);
  g.setColour(getGyeolColor(GyeolPalette::TextPrimary));
  ...
  auto messageBounds = juce::Rectangle<int>(
      static_cast<int>(box.getX() + 26.0f),
      static_cast<int>(box.getY() + 24.0f),
      bounds.getWidth() - 36,
      expanded ? 34 : 28);
  g.setColour(getGyeolColor(GyeolPalette::TextSecondary));
  g.setFont(makeFont(10.3f, false));
  ...
  if (quickFixAvailable) {
    auto fixBounds = juce::Rectangle<float>(box.getRight() - 74.0f, box.getY() + 6.0f, 30.0f, 14.0f);
    g.setColour(getGyeolColor(GyeolPalette::AccentPrimary).withAlpha(0.22f));
    g.fillRoundedRectangle(fixBounds, 3.0f);
    g.setColour(getGyeolColor(GyeolPalette::AccentPrimary).withAlpha(0.95f));
    g.setFont(makeFont(9.0f, true));
    g.drawText("Fix", fixBounds.toNearestInt(), juce::Justification::centred, false);
  }

  auto muteBounds = juce::Rectangle<float>(box.getRight() - 40.0f, box.getY() + 6.0f, 32.0f, 14.0f);
  g.setColour(getGyeolColor(muted ? GyeolPalette::TextDisabled : GyeolPalette::HeaderBackground)
                  .withAlpha(0.85f));
  g.fillRoundedRectangle(muteBounds, 3.0f);
  g.setColour(getGyeolColor(GyeolPalette::TextSecondary).withAlpha(0.94f));
  g.setFont(makeFont(8.8f, true));
  g.drawText(muted ? "Muted" : "Mute", muteBounds.toNearestInt(), juce::Justification::centred, false);
}

void GyeolCustomLookAndFeel::drawPerformanceDashboardButton(
    juce::Graphics &g,
    ...
    bool pressed,
    bool enabled) const {
  auto box = bounds.reduced(0.6f);
  auto base = getGyeolColor(GyeolPalette::ControlBase);
  auto lightEdge = getGyeolColor(GyeolPalette::PanelBackground).interpolatedWith(juce::Colours::white, 0.35f);
  auto darkEdge = getGyeolColor(GyeolPalette::BorderDefault).interpolatedWith(juce::Colours::black, 0.35f);

  if (toggled)
    base = base.interpolatedWith(getGyeolColor(GyeolPalette::AccentPrimary), 0.28f);
  if (highlighted)
  ...
  if (!enabled)
    base = getGyeolColor(GyeolPalette::ControlDisabled);

  g.setColour(base.withAlpha(enabled ? 0.98f : 0.72f));
  g.fillRoundedRectangle(box, 7.0f);

  g.setColour(lightEdge.withAlpha(0.50f));
  g.drawRoundedRectangle(box.reduced(0.4f), 7.0f, 1.0f);

  g.setColour(darkEdge.withAlpha(0.60f));
  g.drawRoundedRectangle(box.translated(0.0f, 0.9f), 7.0f, 1.0f);

  g.setColour(getGyeolColor(enabled ? GyeolPalette::TextPrimary : GyeolPalette::TextDisabled)
                  .withAlpha(toggled ? 1.0f : 0.92f));
  g.setFont(makeFont(10.2f, true));
  g.drawText(text, box.toNearestInt(), juce::Justification::centred, true);
}
// ==============================================================================
