...
public:
  ...
  void paintWidget(juce::Graphics &g, const WidgetModel &widget,
    ...
    if (selected) {
      // Phase 3: 네온 Glow 외곽선 (바깥→안으로 여러 레이어)
...
public:
  ...
  void paint(juce::Graphics &g) override {
    ...
    if (snapSettings.enableGrid && visibleCanvasBounds.getWidth() > 0.0f &&
      ...
      // Phase 3: 줌 레벨에 따른 그리드 페이드아웃
      // 줌이 작아질수록 그리드가 옅어져서 시각적 노이즈를 줄임
      const auto gridFade =
          juce::jlimit(0.0f, 1.0f,
                       (zoomLevel - 0.3f) / 0.5f); // 0.3 이하에서 완전 투명
...
public:
  explicit Impl(EditorHandle &ownerIn)
    ...
    leftPanels.addTab("Layers", juce::Colour::fromRGB(24, 28, 34),
                      &layerTreePanel, false);
    leftPanels.addTab("Library", juce::Colour::fromRGB(24, 28, 34),
    ...
    rightPanels.addTab("Inspector", juce::Colour::fromRGB(24, 28, 34),
                       &propertyPanel, false);
    rightPanels.addTab("Event/Action", juce::Colour::fromRGB(24, 28, 34),
                       &eventActionPanel, false);
    rightPanels.addTab("Validation", juce::Colour::fromRGB(24, 28, 34),
                       &validationPanel, false);
    rightPanels.addTab("Export Preview", juce::Colour::fromRGB(24, 28, 34),
                       &exportPreviewPanel, false);
    rightPanels.addTab("Performance", juce::Colour::fromRGB(24, 28, 34),
  ...
  ~Impl() override { cancelPendingUpdate(); }
  ...
  void paint(juce::Graphics &g, juce::Rectangle<int> bounds) {
    ...
    for (int x : separatorXs) {
      g.drawVerticalLine(x, static_cast<float>(toolbarBounds.getY() + 8),
  ...
  void resized(juce::Rectangle<int> bounds) {
    ...
    auto addSeparator = [&toolbar, this]() {
      toolbar.removeFromLeft(4);
    ...
    for (auto &entry : createButtons) {
      place(*entry.button, 36);
    }

    addSeparator();

    // Group 2: Edit & Arrange
    place(undoButton, 36);
    place(redoButton, 36);
    place(deleteSelected, 36);
    place(groupSelected, 36);
    place(ungroupSelected, 36);
    place(arrangeMenuButton, 36);

    addSeparator();

    // Group 3: Export & IO
    place(dumpJsonButton, 44);
    place(exportJuceButton, 44);
    ...
    const auto expandedHistoryDockHeight =
        content.getHeight() > 360 ? historyPanelHeight : 140;
    const auto collapsedHistoryDockHeight = 36;
    const auto historyDockHeight = historyPanel.isCollapsed()
                                       ? collapsedHistoryDockHeight
...
private:
  ...
  void updateShortcutHintForMode() {
    const auto previewText =
        "Preview: Del delete  Ctrl/Cmd+G group  Ctrl/Cmd+Shift+G ungroup  "
        "Ctrl/Cmd+Alt+Arrows/H/V align  Ctrl/Cmd+Alt+Shift+H/V distribute  "
        "Ctrl/Cmd+[ ] layer order  Ctrl/Cmd+Z/Y undo/redo";
