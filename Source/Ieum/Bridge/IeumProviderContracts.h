...
#include "../Document/IeumBindingTypes.h"
#include <vector>
 ...
 */
struct IeumParamInfo {
    ParamId id;                 // 코드용 식별자
    juce::String name;          // UI 출력용 이름
    IeumValueType type = IeumValueType::Floating;
    IeumValue minValue = 0.0;
    IeumValue maxValue = 1.0;
    IeumValue defaultValue = 0.0;
 ...
 */
class IeumSourceProvider {
public:
    virtual ~IeumSourceProvider() = default;
    virtual ProviderId getProviderId() const = 0;

    /** 내보낼 수 있는 모든 소스 목록을 조사합니다. */
    virtual std::vector<IeumEntityInfo> getAvailableSources() = 0;
 ...
 */
class IeumTargetProvider {
public:
    virtual ~IeumTargetProvider() = default;
    virtual ProviderId getProviderId() const = 0;

    /** 저장이 가능한 모든 타겟 목록을 조사합니다. */
    virtual std::vector<IeumEntityInfo> getAvailableTargets() = 0;
